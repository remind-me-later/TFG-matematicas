\documentclass[a4paper, 10pt, parskip=half, DIV=20, toc=bibliography]{scrartcl}
...
\[
  \texttt{add}_1 \ (\texttt{var}_1 \ ``x'') \ (\texttt{val}_1 \ 3)
\]
...
\begin{theorem}[Map evaluates to last]
  If we assign a value to a variable, and then assign another value to the same
  variable, the map evaluates to the last value assigned.
  \[
...
All of this can be denoted in Lean as:
\begin{minted}{lean4}
...
And the example~\ref{ex:arithmetic_evaluation} can be written as:
\begin{minted}{lean4}
...
The example~\ref{ex:arithmetic_evaluation} can the be directly 
evaluated in Lean as:
\begin{minted}{lean4}
...
\]
we have to introduce an intermediate state \(s_2\) that doesn't appear in the
final result, in this way we account for \enquote{intermediate steps} in the 
evaluation of the \texttt{while} construct. While the evaluation rules for
...
The encoding of the big step semantics of commands in Lean is:
\begin{minted}{lean4}
...
\end{minted}
  to simplify the notation we will denote the program as \(P\), and the
  the all the if construct as \(\texttt{Ifelse}\), then:
  \[
  \texttt{cat}_n: \frac{
...
In the previous theorem we have to be careful to not confuse the \enquote{if}
