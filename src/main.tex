\documentclass[a4paper, 10pt, parskip=half, DIV=24, toc=bibliography]{scrartcl}
...
\[
\texttt{if}_{0n} : \frac{b \ s_1 = \texttt{false}  \quad (c_1, s_1) \Longrightarrow_n s_2}
{(\texttt{if} \ b \ \texttt{then} \ c_1 \ \texttt{else} \ c_2 \ \texttt{end}, s_1) \Longrightarrow_n s_2}
\]
and another for the case when the condition is true:
\[
\texttt{if}_{1n} : \frac{b \ s_1 = \texttt{true}  \quad (c_2, s_1) \Longrightarrow_n s_2}
{(\texttt{if} \ b \ \texttt{then} \ c_1 \ \texttt{else} \ c_2 \ \texttt{end}, s_1) \Longrightarrow_n s_2}
\]
...
\texttt{while}_{0n} : \frac{b \ s = \texttt{false}}
  {(\texttt{while} \ b \ \texttt{loop} \ c \ \texttt{end}, s) \Longrightarrow_n s}
\]
...
\[
\texttt{while}_{1n} : \frac{b \ s_1 = \texttt{true} \quad (c, s_1) \Longrightarrow_n s_2 \quad
  (\texttt{while} \ b \ \texttt{loop} \ c \ \texttt{end}, s_2) \Longrightarrow_n s_3}
{(\texttt{while} \ b \ \texttt{loop} \ c \ \texttt{end}, s_1) \Longrightarrow_n s_3}
\]
...
\end{minted}
  ...
  \[
  \texttt{cat}_n: \frac{
    \texttt{ass}_n: (x := 2, s_0) \Longrightarrow_n s_0[x \leftarrow 2]
    \quad
    \texttt{if}_{0n}: \displaystyle{\frac{
      \texttt{ass}_n: (z := 4, s_0[x \leftarrow 2]) \Longrightarrow_n s_0[x \leftarrow 2][z \leftarrow 4]
    }
    {
      (\text{Ifelse}, s_0[x \leftarrow 2]) \Longrightarrow_n s_0[x \leftarrow 2][z \leftarrow 4]
    }}
  }{
    (P, s_0) \Longrightarrow_n s_0[x \leftarrow 2][z \leftarrow 4]
  }
  \]
\end{example}
...
\begin{theorem}[Skip equality]
  ...
  \[
  ((\texttt{skip}, s_1) \Longrightarrow_n s_2) =  (s_1 = s_2)
  \]
\end{theorem}
...
\begin{theorem}[Concatenation equality]
  ...
  \[
  ((c_1 \texttt{;} c_2, s_1) \Longrightarrow_n s_3) = 
  \exists s_2, \left((c_1, s_1) \Longrightarrow_n s_2 \land (c_2, s_2) \Longrightarrow_n s_3\right)
  \]
\end{theorem}
...
\begin{theorem}[If equality (version 1)]\label{thm:if_rewrite_1}
  ...
  \[
  ((\texttt{if} \ b \ \texttt{then} \ c_1 \ \texttt{else} \ c_2 \ \texttt{end}, s_1) \Longrightarrow_n s_2) = 
  \text{if} \ b \ s_1 \ \text{then} \ (c_1, s_1) \Longrightarrow_n s_2 \ \text{else} \ (c_2, s_1) \Longrightarrow_n s_2
  \]
\end{theorem}
or, equivalently:
\begin{theorem}[If equality (version 2)]\label{thm:if_rewrite_2}
  ...
  \[
  ((\texttt{if} \ b \ \texttt{then} \ c_1 \ \texttt{else} \ c_2 \ \texttt{end}, s_1) \Longrightarrow_n s_2) =
  ((\text{if} \ b \ s_1 \ \text{then} \ c_1 \ \text{else} \ c_2) \Longrightarrow_n s_2)
  \]
\end{theorem}
...
\begin{theorem}[While equality]\label{thm:while_rewrite}
  ...
  &((\texttt{while} \ b \ \texttt{loop} \ c \ \texttt{end}, s_1) \Longrightarrow_n s_3) = \\
  &\quad \text{if} \ b \ s_1 \\
  &\quad \text{then} \ \exists s_2, ((c, s_1) \Longrightarrow_n s_2) \land ((\texttt{while} \ b \ \texttt{loop} \ c \ \texttt{end}, s_2) \Longrightarrow_n s_3) \\
...
\begin{definition}[Behavioral equivalence]
  ...
  \[
  c_1 \sim c_2 \coloneq \forall s_1, \forall s_2, (c_1, s_1) \Longrightarrow_n s_2 \iff (c_2, s_1) \Longrightarrow_n s_2
  \]
\end{definition}
...
% The proof of this fact is quite interesting, so we reproduce it in full:
% \begin{minted}{lean4}
% theorem loop_unfold:
%   while b loop c end ≈
