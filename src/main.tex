...
\begin{itemize}
  \item[\(\alpha\)-conversion] Allows us to rename bound variables:
    \((\lambda x \implies M) \ N \longrightarrow M[x := N]\) 
  ...
  \item[\(\beta\)-reduction] Allows us to apply a function to an argument, and
    ...
    \((\lambda x \implies M) \ N \longrightarrow M[x \coloneq N]\).
  ...
  \item[\(\eta\)-conversion] Expresses the concept of \emph{extensionality} of functions, 
    ...
    To give an example it converts \((\lambda x \implies f \ x) \longrightarrow f\)
...
\begin{example}
  ...
  \[
  (\lambda x \implies x + 1) \ 3 \longrightarrow 3 + 1
  \]
\end{example}
...
\enquote{reduce} expressions to simpler ones, but this is not always the case.

\begin{definition}[Normal form]
  An expression is in normal form if it cannot be reduced further.
\end{definition}

Now we present an example of an expression that does not have a normal form.

\begin{definition}[\(\Omega\)-combinator]\label{def:omega_combinator}
  The \(\Omega\)-combinator:
  \[
  \Omega \coloneq (\lambda x \implies x \ x) \ (\lambda x \implies x \ x)
  \]
  is a lambda expression that does not have a normal form
\end{definition}

\begin{proof}
  If we apply the \(\beta\)-reduction rule to~\ref{def:omega_combinator}we get
  the expression itself:
  \[
  (\lambda x \implies x \ x) \ (\lambda x \implies x \ x) \longrightarrow
  (\lambda x \implies x \ x) \ (\lambda x \implies x \ x)
  \]
  So the expression does not have a normal form.
\end{proof}
...
\begin{definition}[Map update]
  ...
  \funfont{update} \ & (m: \tyfont{Map} \ \alpha) \ (k: \tyfont{String}) \ (v: \alpha): \tyfont{Map} \ \alpha \coloneq \\
                      &\lambda k' \implies \funfont{cond} \ (k = k') \ v \ (m \ k')
  \end{align*}
\end{definition}
...
\begin{align*}
  \funfont{update} \ & (m: \tyfont{Map} \ \alpha) \ (k: \tyfont{String}) \ (v: \alpha): \tyfont{Map} \ \alpha \coloneq \\
                      &\lambda k' \implies \text{if} \ k = k' \ \text{then} \ v \ \text{else} \ m \ k'
\end{align*}
...
\begin{definition}[Initial state]
  ...
  \[
  s_0 \coloneq \lambda x \implies 0
  \]
\end{definition}
...
\[
  \texttt{add}_n : \frac{(a_1, s) \bigstep n_1 \quad (a_2, s) \bigstep n_2}
...
\begin{align*}
  \texttt{sub}_n &: \frac{(a_1, s) \bigstep n_1 \quad (a_2, s) \bigstep n_2}
   {(a_1 - a_2, s) \bigstep n_1 - n_2} \\
  \texttt{mul}_n &: \frac{(a_1, s) \bigstep n_1 \quad (a_2, s) \bigstep n_2}
   {(a_1 * a_2, s) \bigstep n_1 * n_2}
\end{align*}
...
\[
  \texttt{add}_n: \frac{
    \texttt{add}_n: \displaystyle{\frac{
      \texttt{var}_n: (x, s_0) \bigstep 0
      \quad
      \texttt{val}_n: (5, s_0) \bigstep 5
    }{(x + 5, s_0) \bigstep 5}}
    \quad 
    \texttt{sub}_n: \displaystyle{\frac{
      \texttt{val}_n: (9, s_0) \bigstep 9
      \quad
      \texttt{val}_n: (7, s_0) \bigstep 7
    }{(9 - 7, s_0) \bigstep 2}}
  }
...
\begin{align*}
  \forall n, a_1 \ s &= n \implies (a_1, s) \bigstep n \\
...
\begin{align*}
  \texttt{true}_n &: (\texttt{true}, \cdot) \bigstep \texttt{true} \\
  \texttt{false}_n &: (\texttt{false}, \cdot) \bigstep \texttt{false} \\
  \texttt{not}_n &: \frac{(b, s) \bigstep b'}{\lnot (b, s) \bigstep \lnot b'} \\
  \texttt{and}_n &: \frac{(b_1, s) \bigstep b_1' \quad (b_2, s) \bigstep b_2'}
    {(b_1 \land b_2, s) \bigstep b_1' \land b_2'} \\
  \texttt{or}_n &: \frac{(b_1, s) \bigstep b_1' \quad (b_2, s) \bigstep b_2'}
    {(b_1 \lor b_2, s) \bigstep b_1' \lor b_2'} \\
  \texttt{eq}_n &: (a_1 = a_2, s) \bigstep a_1 \ s = a_2 \ s \\
  \texttt{le}_n &: (a_1 \leq a_2, s) \bigstep a_1 \ s \leq a_2 \ s
\end{align*}
...
\begin{align*}
  \texttt{skip}_n &: (\texttt{skip}, s) \bigstep s \\
  \texttt{ass}_n &: (v \ \texttt{:=} \ a, s) \bigstep s[v \leftarrow a \ s] \\
  \texttt{cat}_n &: \frac{
...
\[
\texttt{if}_{0n} : \frac{b \ s_1 = \texttt{false}  \quad (c_1, s_1) \bigstep s_2}
{(\texttt{if} \ b \ \texttt{then} \ c_1 \ \texttt{else} \ c_2 \ \texttt{end}, s_1) \bigstep s_2}
\]
and another for the case when the condition is true:
\[
\texttt{if}_{1n} : \frac{b \ s_1 = \texttt{true}  \quad (c_2, s_1) \bigstep s_2}
{(\texttt{if} \ b \ \texttt{then} \ c_1 \ \texttt{else} \ c_2 \ \texttt{end}, s_1) \bigstep s_2}
\]
...
\[
\texttt{while}_{1n} : \frac{b \ s_1 = \texttt{true} \quad (c, s_1) \bigstep s_2 \quad
...
\end{verbatim}
  ...
  \[
  \texttt{cat}_n: \frac{
    \texttt{ass}_n: (x := 2, s_0) \bigstep s_0[x \leftarrow 2]
    \quad
    \texttt{if}_{0n}: \displaystyle{\frac{
      \texttt{ass}_n: (z := 4, s_0[x \leftarrow 2]) \bigstep s_0[x \leftarrow 2][z \leftarrow 4]
    }
    {
...
\begin{definition}[Behavioral equivalence]
  ...
  \[
  c_1 \sim c_2 \coloneq \forall s_1, \forall s_2, (c_1, s_1) \bigstep s_2 \iff (c_2, s_1) \bigstep s_2
  \]
\end{definition}
...
\begin{proof}
  ...
  The rest of the cases are analogous, the details can be found in the code.
\end{proof}
