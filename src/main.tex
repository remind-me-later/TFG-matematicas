\documentclass[
  a4paper, 
  10pt, 
  parskip=half, 
  % DIV=10,
  toc=bibliography,
  % toc=flat,
  % draft,
]{scrbook}

% \usepackage{showframe}

\usepackage{polyglossia}
\setdefaultlanguage{spanish}

\usepackage{epigraph}

\usepackage{graphicx}
\graphicspath{ {../resources/} }

\usepackage{fontspec}
% switch to a monospace font supporting more Unicode characters
\setmonofont{JetBrains Mono}[
    Contextuals=Alternate,
    Ligatures=TeX,
    Scale=MatchLowercase,
]

\usepackage{minted}
\usemintedstyle{pastie}

% === Math facilities ===

% Theorem environments
\usepackage{amsmath}
\usepackage{amsthm}
\usepackage{amssymb}
\usepackage{centernot}
\usepackage{mathtools}

\theoremstyle{definition}

% Custom theoremstyle: like definition, but theorem names in bold
\newtheoremstyle{bolddef}
  {3pt} % Space above
  {3pt} % Space below
  {\normalfont} % Body font
  {} % Indent amount
  {\bfseries} % Theorem head font (bold)
  {.} % Punctuation after theorem head
  { } % Space after theorem head
  {\thmname{#1}\thmnumber{ #2}\thmnote{ (#3)}} % Theorem head spec

\theoremstyle{bolddef}

\newtheorem{theorem}{Teorema}[section]
% \newtheorem{proposition}[theorem]{Proposition}
\newtheorem{corollary}[theorem]{Corolario}
\newtheorem{lemma}[theorem]{Lema}

\newtheorem{definition}[theorem]{Definición}
% \newtheorem{remark}[theorem]{Remark}
\newtheorem{notation}[theorem]{Notación}

\newtheorem{example}[theorem]{Ejemplo}


\addto\captionsspanish{\renewcommand\proofname{Demostración}}

\newcommand{\funfont}[1]{\texttt{#1}}
\newcommand{\tyfont}[1]{\texttt{\textbf{#1}}}
\renewcommand{\implies}{\rightarrow}
\renewcommand{\impliedby}{\leftarrow}
\renewcommand{\iff}{\leftrightarrow}
\renewcommand{\mapsto}{\Rightarrow}
\newcommand{\Prop}{\mathbf{Prop}}
\newcommand{\placeholder}{\square}

\newcommand{\ImpIfElse}[3]{\texttt{if} \ {#1} \ \{{#2}\} \ \texttt{else} \ \{{#3}\}}
\newcommand{\ImpWhile}[2]{\texttt{while} \ {#1} \ \{{#2}\}}
\newcommand{\ImpConcat}[2]{{#1} \texttt{;} \ {#2}}
\newcommand{\ImpSkip}{\texttt{skip}}
\newcommand{\ImpAssign}[2]{{#1} = {#2}}

\newcommand{\LeanIfElse}[3]{\text{if} \ {#1} \ \text{then} \ {#2} \ \text{else} \ {#3}}
\newcommand{\LeanTrue}{\text{true}}
\newcommand{\LeanFalse}{\text{false}}

\newcommand{\ImpPlus}{\mathbin{\texttt{+}}}
\newcommand{\ImpMinus}{\mathbin{\texttt{-}}}
\newcommand{\ImpTimes}{\mathbin{\texttt{*}}}

\newcommand{\ImpNot}{\texttt{!}}
\newcommand{\ImpAnd}{\mathbin{\texttt{\&\&}}}
\newcommand{\ImpOr}{\mathbin{\texttt{||}}}
\newcommand{\ImpEq}{\mathbin{\texttt{==}}}
\newcommand{\ImpLe}{\mathbin{\texttt{<=}}}

\newcommand{\ImpTrue}{\texttt{true}}
\newcommand{\ImpFalse}{\texttt{false}}

\newcommand{\ImpName}{\texttt{IMP}}

\usepackage[only,llbracket,rrbracket]{stmaryrd}

% define math symbol for big step arrow
\newcommand{\bigstep}{\Longrightarrow}
\newcommand{\Denote}[1]{\llbracket{} {#1} \rrbracket{}}

% === End math facilities ===

\usepackage{csquotes}

\usepackage{bookmark}
\hypersetup{
    colorlinks=true,
    % linkcolor=blue,
    % filecolor=magenta,      
    % urlcolor=cyan,
    % pdftitle={Overleaf Example},
    }

% main matter no reinicia el contador de páginas
\makeatletter
\renewcommand\mainmatter{
    \@mainmattertrue\cleardoublepage\renewcommand\thepage{\arabic{page}}}
\makeatother

\providecommand{\keywords}[1]
{
  \small	
  \textbf{\textit{Palabras clave---}} #1
}

\renewenvironment{dedication}
  {\clearpage           % we want a new page
   \thispagestyle{empty}% no header and footer
   \vspace*{\stretch{1}}% some space at the top 
  % \section*{Dedication} % <==========================================
   \itshape             % the text is in italics
   \raggedleft{}          % flush to the right margin
  }
  {\/
   \par % end the paragraph
   \vspace{\stretch{3}} % space at bottom is three times that at the top
   \clearpage           % finish off the page
  }

\begin{document}

\frontmatter

\begin{titlepage}
  \vspace*{\stretch{1}}
  \begin{center}
      {\LARGE \textsc{Presentación con Lean 4 de algunos Resultados clásicos sobre
      Semántica de Lenguajes de Programación} \bigbreak}
      \includegraphics[
        width=0.5\textwidth,
        keepaspectratio,
      ]{ucmlogo.png}
      \bigbreak{}
      {\Large \textsc{Ricardo Maurizio Paul} \bigbreak}
      {
        \large
        \textsc{Facultad de Ciencias Matemáticas}
        \bigbreak}
      {\large Trabajo de Fin de Grado presentado para optar al \emph{Grado en Matemáticas}
      \bigbreak}
      {\large \textsc{Tutor: David de Frutos Escrig} \bigbreak}
      {\large \textsc{Junio de 2025}}
      \vspace*{\stretch{2}}
  \end{center}
\end{titlepage}

\makeatletter\@openrightfalse{}
\chapter*{Resumen}

La utilización de Asistentes para la Demostración de resultados matemáticos parece que terminará por
constituirse en una absoluta necesidad a fin de poder ratificar la corrección de pruebas previamente
realizadas “manualmente”. Además, la incorporación de esas pruebas formales a los bancos de la
aplicación con la que se hayan realizado permitirá tanto la utilización de los resultados probados con total
garantía de calidad, como el cotejo de los procedimientos utilizados en su obtención, que darán lugar a
metodologías para el futuro desarrollo de nuevas pruebas a través de patrones o “tácticas”, como se les
suele llamar en el argot.

La formalización de las Semánticas de los Lenguajes de Programación constituye un interesante campo de
aplicación de estos desarrollos, por varias razones. En una primera aproximación parece un campo
particularmente adecuado, pues las citadas semánticas se definen con elementos lógicos y algebraicos
básicos que auguran una relativamente fácil formalización total. Sin embargo, cuando se entra en el detalle
aparecen dificultades inesperadas cuya resolución sacará a la luz las técnicas necesarias para tratar con
éxito casos como estos. Al tiempo, pese a que las componentes matemáticas son evidentes, de nuevo la
forma concreta en que aparecen y se combinan se separa un tanto de la habitual en campos puramente
algebraicos, por lo que se han de crear nuevas técnicas para lidiar con ellas.

En este trabajo nos proponemos formalizar las semánticas operacional y denotacional de un
lenguaje de programación imperativo sencillo, estudiar algunas de sus propiedades y
finalmente demostrar la equivalencia entre las diferentes semánticas.

\chapter*{Abstract}

The use of Proof Assistants for mathematical proofs seems destined to become an absolute necessity in order to 
ratify the correctness of proofs previously carried out \``manually''. Furthermore, the incorporation of these formal 
proofs into the databases of the application with which they were made will allow both the use of the proven results 
with a total guarantee of quality, as well as the comparison of the procedures used to obtain them, 
which will give rise to methodologies for the future development of new proofs through patterns or ``tactics'', 
as they are usually called in the jargon.

The formalization of the Semantics of Programming Languages constitutes an interesting field of application for these 
developments for several reasons. It seems a particularly suitable field, since the 
aforementioned semantics are defined with basic logical and algebraic elements that suggest a relatively easy 
total formalization. However, when one goes into detail, unexpected difficulties arise whose resolution will bring to 
light the necessary techniques to successfully deal with cases like these. At the same time, although the mathematical 
components are evident, again the concrete way in which they appear and combine differs somewhat from the usual in 
purely algebraic fields, so new techniques must be created to deal with them.

In this work, we propose to formalize the operational and denotational semantics of a simple imperative programming 
language, study some of its properties, and finally demonstrate the equivalence between the different semantics.
\@openrighttrue\makeatother 

\begin{dedication}
  Para todos los que han hecho posible este trabajo y en especial para Claudia.
\end{dedication}

\tableofcontents

\chapter{Antecedentes históricos}

Los asistentes de pruebas (o \emph{proof assistants}) son sistemas de software que ayudan en la
construcción y verificación de demostraciones matemáticas mediante el uso de
ordenadores. La creación de asistentes de pruebas se remonta a los años 50 y 60, con
el sistema \emph{Logic Theorist}\footnote{\url{https://en.wikipedia.org/wiki/Logic_Theorist}} (1956). 
Este el primer programa diseñado para realizar \enquote{razonamiento automático} y
fue capaz de demostrar teoremas algunos de la lógica proposicional y de la de predicados. 
Su creación precede a la teoría de tipos moderna y 
al isomorfismo de Curry-Howard, por lo que usaban una
lógica diferente basada en la \emph{deducción natural} y la \emph{lógica de primer orden}
(\emph{first-order logic} o \emph{FOL}).\@ El sistema fue capaz de demostrar teoremas
interesantes, pero tenía un alcance limitado. A grandes rasgos, el sistema
generaba un \emph{árbol de búsqueda} de todas las posibles demostraciones y
las evaluaba para encontrar una demostración válida, de forma similar a 
como trabaja Prolog.

El sistema \emph{Automath} (1967) de De Bruijn fue uno de los primeros
sistemas en usar una teoría de tipos y el isomorfismo de Curry-Howard, que fue 
descubierto independientemente por el propio De Bruijn.

En los años 70 se desarrollaron sistemas más potentes, como \emph{Logic for Computable Functions}, 
basado en la \emph{Lógica de Funciones Computables} o \emph{LCF} de Dana Scott. Este fue uno de los
primeros sistemas en usar demostraciones por inducción, una característica fundamental
para razonar sobre funciones definidas recursivamente, y una de las técnicas 
fundacionales de los asistentes de pruebas modernos. La creación del
lenguaje de programación ML, el padre de todos los lenguajes funcionales, también
se atribuye al desarrollo de LCF, ya que fue diseñado para escribir \emph{tácticas}
en ducho demostrador. El enfoque moderno de construcción de pruebas \enquote{paso a paso},
usado por todos los asistentes modernos, también se remonta a LCF.\@

En los años 70 también se refinaron ideas como la teoría de tipos. En
particular la teoría de tipos de Martin-Löf, y el isomorfismo de Curry-Howard,
dos conceptos clave que son la base de todos los asistentes de pruebas modernos.
En los años 80 estas ideas fueron implementadas en sistemas como \emph{Nuprl} y
\emph{Coq}\,.

Nuprl es uno de los primeros sistemas en usar una teoría de tipos
constructiva en su núcleo, donde las demostraciones son objetos de primera clase,
enfatizando su contenido computacional y permitiendo la \emph{extracción de
código} de programas a partir de demostraciones verificadas. Nuprl también
introdujo tipos dependientes, una característica poderosa que permite que el
tipo de un valor dependa del propio valor. Con este sistema se pudieron demostrar
formalmente resultados sobre la paradoja de Girard y fue usado también para
demostrar la corrección de protocolos de coherencia de caché.

Un poco más tarde llegó Coq, un sistema basado en el CIC (Cálculo de Construcciones
Inductivas), una teoría de tipos que extiende el Cálculo de Construcciones 
(\emph{Calculus of Constructions} o \emph{CoC}) con tipos inductivos. Coq es uno de los
asistentes de pruebas más populares, y se continúa utilizando ampliamente en la academia y
la industria a día de hoy. Veremos que el CIC también es la base de Lean, un
asistente de pruebas más reciente que ha ganado una popularidad significativa en
los últimos años.

Al mismo tiempo, se desarrolló otra línea de investigación, la familia de
asistentes de pruebas basados en \emph{HOL} (\emph{Higher Order Logic}), una lógica que
extiende la lógica de primer orden con cuantificadores de orden superior. Estos
sistemas están basados en la arquitectura \emph{LCF} y utilizan un \emph{núcleo} para
asegurar la corrección de las pruebas. El sistema más popular de esta familia es
\emph{Isabelle}, un asistente de pruebas tan potente como Coq, pero con una
fundamentación lógica diferente. 

Pero volvamos a Lean, el asistente de pruebas que usaremos en este texto. Lean
es un asistente de pruebas moderno desarrollado por Leonardo de Moura en Microsoft
Research basado en el CIC.\@ Podemos ver Lean como una evolución de Coq, con una
sintaxis más moderna, y un sistema de tipos más potente. Más adelante discutiremos
las principales diferencias con cierto detalle. Otra característica interesante de Lean es que 
su comunidad ha creado una biblioteca de matemáticas muy rica, conocida como \emph{mathlib}, 
donde podemos encontrar resultados sobre teoría de números, álgebra, teoría de
categorías, cálculo, y mucho más.

Dejando de lado los sistemas basados en HOL, todos estos sistemas están basados en el
isomorfismo de Curry-Howard, un concepto fundamental que relaciona demostraciones
y programas. Para entenderlo, primero debemos introducir los conceptos básicos de
la teoría de tipos.

\mainmatter{}

\chapter{Teoría de tipos de Lean}

La teoría de tipos es una rama de la lógica matemática que se ocupa del estudio
de los tipos, una abstracción similar en algunos aspectos a los conjuntos, que 
sin duda será más familiar a el lector medio. La teoría de tipos surgió a principios del siglo XX
para resolver la crisis de los fundamentos de las matemáticas, en concreto una serie de
problemas que surgieron a raíz del descubrimiento de paradojas en la teoría de
conjuntos, como la paradoja de Russell.

De la misma forma que los \emph{conjuntos} son una noción primitiva en la teoría de conjuntos,
los \emph{tipos}, junto con las \emph{funciones}, son la noción primitiva de la teoría de tipos.
La idea principal detrás de los tipos es clasificar los objetos matemáticos de
forma que las funciones solo puedan aplicarse a objetos de un tipo determinado,
lo que se conoce como \emph{seguridad de tipos} (\emph{type-safety}). 
Los programadores reconocerán este concepto, ya que es la base de los sistemas de tipos
estáticos en lenguajes de programación como Haskell, Rust y Scala.

Como introducción informal consideramos el siguiente ejemplo: definimos el tipo
\(\mathbb{N}\) que representa los números naturales, y una función
\(\texttt{add}: \mathbb{N} \to \mathbb{N} \to \mathbb{N}\) que toma dos números
naturales y devuelve su suma\footnote{Más adelante formalizaremos la sintaxis de los tipos y funciones.}. 
Esto asegura que la función \(\texttt{add}\) solo puede aplicarse a valores de 
tipo \(\mathbb{N}\), y no a las de ningún otro tipo, y que el resultado de la aplicación
también es de tipo \(\mathbb{N}\).

Históricamente los orígenes de la teoría de tipos se remontan a múltiples
autores como Church, Curry, Russell, y otros. Pero la teoría de tipos moderna se basa en el
trabajo de Pier Martin-Löf, quien desarrolló una teoría de tipos que combina la
teoría de tipos clásica con el \(\lambda\)-cálculo. De esta manera las
\enquote{buenas} propiedades de las teorías de tipos anteriores reciben una
\emph{interpretación computacional}, a través del isomorfismo de Curry-Howard,
que nos permitirá razonar sobre demostraciones matemáticas. 

La teoría de tipos de Martin-Löf es \emph{intuicionista}, es decir, las
demostraciones no son solo una argumentación lógica sino un método para
construir un \emph{testigo} de la verdad de una proposición. Esto contrasta con la lógica
clásica, donde las demostraciones son solo una secuencia de pasos lógicos que
muestran la verdad de una proposición. En la lógica intuicionista, el principio del
tercio excluido no es válido, como tampoco lo es la \emph{reducción al absurdo} (con
algunas salvedades).

\section{Lambda cálculo}

Un desarrollo detallado del \(\lambda\)-cálculo se puede encontrar en~\cite{barendregt1984lambda},
pero por completitud haremos aquí una breve introducción.
El \(\lambda\)-cálculo es un sistema formal para expresar \emph{computación} basado en la
abstracción y aplicación de funciones. 
Fue creado por Alonzo Church en la década de 1930 para desarrollar los fundamentos de las matemáticas y
la computación.
El \(\lambda\)-cálculo es, en esencia, un \enquote{lenguaje de programación} con un solo tipo de datos, las funciones, y
dos operaciones, la abstracción y la aplicación.

En el \(\lambda\)-cálculo, las funciones son vistas como \emph{reglas de cómputo} en vez de como
\emph{relaciones}, lo cual enfatiza su naturaleza computacional, y
nos permite razonar sobre la computación en general de una forma más abstracta. Esto contrasta
con la teoría de conjuntos, donde las funciones son vistas \emph{extensionalmente},
como una particularización de las relaciones.

La sintaxis del \(\lambda\)-cálculo es simple, consiste en variables (\(x, y, z\)),
abstracción (\(\lambda x \mapsto M\)), y aplicación (\(M \ M\)). En 
\emph{forma de Backus-Naur} (\emph{BNF}):
\[
  M \Coloneqq x \mid \lambda x \mapsto M \mid M \ M
\]
La notación (\(\mapsto\)), en vez del tradicional (.), no es accidental, y revela una conexión
profunda entre la abstracción de funciones y la implicación lógica, una conexión que exploraremos más adelante.

\begin{example}\label{ex:lambda_expr}
Podemos aplicar la función \emph{sucesor} al número \(3\) como:
\(
(\lambda x \mapsto x + 1) \ 3
\).
\end{example}

Las reglas para evaluar expresiones del \(\lambda\)-cálculo se conocen como
\emph{reglas de reducción} y son:
\begin{itemize}
  \item Sustitución: Permite reemplazar todas las ocurrencias \emph{libres} de la
    variable \(x\) en la expresión \(M\) con la expresión \(N\), y se denota:
    \(M[x \coloneqq N]\).
  \item \(\alpha\)-conversión: Permite renombrar variables ligadas,
    por ejemplo en la expresión \(\lambda x \mapsto x\) podemos cambiar \(x\) por
    \(y\) para obtener \(\lambda y \mapsto y\). Esta operación es importante para evitar
    \enquote{colisiones} de nombres, pero hay que tener cuidado, ya que las reglas
    de renombramiento no son triviales. Por ejemplo, la expresión
    \(\lambda x \mapsto \lambda x \mapsto x\) se puede \(\alpha\)-convertir a
    \(\lambda y \mapsto \lambda x \mapsto x\), pero no a
    \(\lambda y \mapsto \lambda x \mapsto y\)\footnote{
      El problema de comprobar si dos expresiones son equivalentes bajo
      \(\alpha\)-conversión se puede resolver mediante los \emph{índices de De Bruijn}.
      Aplicando esta transformación, la \(\alpha\)-equivalencia es la
      equivalencia sintáctica.
    }. Es el concepto equivalente
    al \emph{shadowing} en lenguajes imperativos.

  \item \(\beta\)-reducción: Permite aplicar una función a un argumento,
    se define mediante la sustitución:
    \[
    (\lambda x \mapsto M) \ N \rightsquigarrow M[x \coloneqq N]
    \]
  \item \(\eta\)-conversión: Expresa el concepto de \emph{extensionalidad} de funciones,
    esto es, dos funciones son iguales si y solo si devuelven el mismo resultado
    para todos los argumentos. Por ejemplo, cuando \(x\) no es libre en \(f\), 
    podemos aplicar la \(\eta\)-reducción:
    \[
    (\lambda x \mapsto f \ x) \rightsquigarrow f
    \]
    En el sentido contrario podemos aplicar \(\eta\)-expansión a \(f\) para
    obtener nuevamente la expresión anterior:
    \[
    f \rightsquigarrow (\lambda x \mapsto f \ x) \ x
    \]
\end{itemize}

\begin{example}
  Podemos aplicar \(\beta\)-reducción a la expresión del Ejemplo~\ref{ex:lambda_expr}:
  \[
  (\lambda x \mapsto x + 1) \ 3 \rightsquigarrow 3 + 1 
  \]
\end{example}

Las reglas de reducción son \enquote{unidireccionales}, es decir, van de izquierda a
derecha. Viendo el ejemplo anterior uno podría pensar que también \enquote{reducen}
expresiones a versiones \enquote{más simples}: aplicando las reglas una tras otra
llegaríamos a una versión de la expresión en la que las reglas de reducción no
se pueden seguir aplicando. Esta es la llamada \emph{forma normal} de una expresión.

\begin{definition}[Forma normal]
  Una expresión está en \emph{forma normal} si no se puede reducir más.
\end{definition}

Lamentablemente, no todas las expresiones tienen una forma normal, como muestra el
siguiente ejemplo particularmente importante.

\begin{definition}[Combinador \(\Omega\)]\label{def:omega_combinator}
  El combinador \(\Omega\) es la expresión \(\lambda\):
  \[
  \Omega \coloneqq (\lambda x \mapsto x \ x) \ (\lambda x \mapsto x \ x) 
  \]
\end{definition}

Sin adentrarnos en los detalles de la definición de la sintaxis y reducción del
\(\lambda\)-cálculo no podemos dar una demostración \enquote{rigurosa} del siguiente
teorema, pero la esbozamos.

\begin{theorem}
  El combinador \(\Omega\) no tiene forma normal.
\end{theorem}

\begin{proof}
  Aplicando \(\beta\)-reducción a la expresión~\ref{def:omega_combinator} obtenemos la
  misma expresión:
  \[
  (\lambda x \mapsto x \ x) \ (\lambda x \mapsto x \ x) \rightsquigarrow
  (\lambda x \mapsto x \ x) \ (\lambda x \mapsto x \ x) \qedhere
  \]
\end{proof}

Como dijimos antes, los únicos \enquote{tipos de datos} presentes en el
\(\lambda\)-cálculo son las funciones, por lo que si queremos utilizar números,
booleans, o cualquier otro tipo de dato, debemos \enquote{codificarlos} como
tales. Veamos una codificación usual de los booleanos, conocida como \emph{codificación de Church}.
En esta codificación los booleanos son funciones que toman dos argumentos y
devuelven uno de ellos. Por ejemplo, podemos definir los literales booleanos como:

\begin{definition}[Booleanos de Church]
  Definimos los literales booleanos:
  \[
    \LeanTrue \coloneqq \lambda x \mapsto \lambda y \mapsto x \qquad
    \LeanFalse \coloneqq \lambda x \mapsto \lambda y \mapsto y 
  \]
\end{definition}

Ahora podemos definir los operadores lógicos \(\texttt{and}\), \(\texttt{or}\), y
\(\texttt{not}\) como funciones que toman dos booleanos y devuelven un booleano.

\begin{definition}[Operadores lógicos]
  Definimos los operadores lógicos:
  \[
    \funfont{and} \coloneqq \lambda x \mapsto \lambda y \mapsto x \ y \ x \quad
    \funfont{or} \coloneqq \lambda x \mapsto \lambda y \mapsto x \ x \ y \quad
    \funfont{not} \coloneqq \lambda x \mapsto x \ \LeanFalse \ \LeanTrue
  \]
\end{definition}

Podemos también definir la estructura de control \emph{if-else}.

\begin{definition}[If-else]
  Definimos la estructura de control \funfont{if-else} como:
  \[
  \funfont{if-else} \coloneqq \lambda b \mapsto \lambda x \mapsto \lambda y \mapsto b \ x \ y 
  \]
\end{definition}

Los naturales también tienen una codificación de Church, pero la omitimos ya que
más adelante veremos una forma más general de codificar tipos de datos (inductivos).

Como hemos visto, el \(\lambda\)-cálculo es un sistema formal extraordinariamente simple, pero muy poderoso,
pudiéndose usar para razonar sobre la computación. De hecho es \emph{Turing
completo}, lo que significa que puede expresar cualquier función computable.

\section{Lambda cálculo tipado}

La simplicidad del \(\lambda\)-cálculo es su mayor virtud, pero como hemos visto también tiene
sus desventajas. Como comentamos al principio nos proponemos crear un sistema de
tipos que impida aplicar funciones a argumentos incorrectos, pero esto es imposible en el 
\(\lambda\)-cálculo. En efecto, como debemos codificar todos los tipos de datos como
funciones, no \enquote{sabemos} en realidad sobre qué estamos evaluando, y peor 
aún, no sabemos si el resultado de una aplicación es una función o una codificación.

Hemos visto además con el combinador \(\Omega\) que el \(\lambda\)-cálculo permite
\emph{autoaplicación} de funciones, lo cual, al menos a priori, imposibilita
darle un tipo a esta expresión.\footnote{
  En algunos sistemas de tipos el combinador \(\Omega\) puede estar bien tipado. Este no es
  el caso en los sistemas de tipos normalmente usados en asistentes de demostración, como el CIC.}.
Presentamos a continuación la familia de \(\lambda\)-cálculos tipados más simples.

\subsection{Lambda cálculo simplemente tipado}

El \emph{cálculo lambda tipado} más simple es el \emph{cálculo lambda simplemente tipado} o
\emph{STLC}. En el STLC cada variable y cada función tienen un tipo y la
abstracción ahora toma la forma \(\lambda x: \alpha \mapsto M\), donde 
\(\alpha\) es el tipo de la variable \(x\). Los dos tipos obligatorios en el STLC
son el tipo función \(\alpha \to \beta\), que representa una función que
toma un argumento de tipo \(\alpha\) y devuelve un valor de tipo \(\beta\); y
el tipo base \(\texttt{Bool}\), que representa los booleanos\footnote{
  En realidad, dependiendo de los \enquote{tipos primitivos} que elijamos, podemos
  definir toda una familia de STLC, por ejemplo, podemos definir un STLC con
  tipos primitivos \(\mathbb{N}\) y \(\texttt{Bool}\), o con tipos primitivos
  \(\mathbb{N}\), \(\texttt{Bool}\) y \(\texttt{String}\).
}.

La sintaxis del STLC es la siguiente:
\begin{align*}
  M \Coloneqq x & \mid \lambda x: \alpha \mapsto M \mid M \ M \\
    & \mid \LeanTrue \mid \LeanFalse \\
    & \LeanIfElse{M}{M}{M}
\end{align*}

Además de las reglas de reducción que vimos antes, debemos añadir las
\emph{reglas de tipado}, que son reglas que definen las expresiones bien
tipadas del cálculo. Estas reglas requieren un \emph{contexto de tipado} que
normalmente se denota como \(\Gamma\), y que es un mapa que asocia variables con
tipos. Estas reglas, por ejemplo, aseguran que la estructura \emph{if-else} se aplica
a booleanos, y que las dos ramas tienen el mismo tipo.

\subsection{Generalización}

El STLC es la base de la mayoría de los \(\lambda\)-cálculos tipados, y se puede
extender permitiendo \emph{términos que dependen de tipos}. Esto se conoce como
\emph{polimorfismo} y el cálculo se conoce como \emph{System F}. También podemos
extender el STLC con \emph{tipos que dependen de otros tipos}, que llamaremos
\emph{constructores de tipos}, dando lugar al \emph{System F\(_\omega\)}.
Si añadimos \emph{tipos dependientes}, obtenemos \(\lambda P\), etc\ldots

Podemos combinar estas extensiones de forma arbitraria para obtener sistemas
con capacidad expresiva diversa. Una taxonomía de estos sistemas viene dada 
por el \(\lambda\)-cubo de Barendregt. En la cúspide de la potencia expresiva 
tenemos CoC, que es la base de Coq y Lean. En lo que queda del texto exploraremos
distintas características del CoC. Para un tratamiento completo del
\(\lambda\)-cubo referimos al lector a~\cite{lambda_with_types}.

\section{Teoría de tipos y teoría de conjuntos}

En esta sección seguiremos la presentación de la teoría de tipos
desarrollada en~\cite{hottbook}, aunque los conceptos presentados son
comunes a todas las teorías de tipos de Martin-Löf. En rigor, no existe 
\emph{una teoría de tipos}, si no varios sistemas formales basados en las mismas ideas.
Las teorías de tipos de Martin-Löf, 
el Cálculo de Construcciones, y el Cálculo de
Construcciones Inductivas son diferentes, pero para los propósitos de este texto
nos referiremos a todas ellas como \emph{teorías de tipos}.

Podemos pensar en el \(\lambda\)-cálculo presentado antes como una \enquote{sintaxis} para
nuestra teoría de tipos: los tipos darán significado a las expresiones del
\(\lambda\)-cálculo. De hecho, una buena parte de las secciones siguientes estarán
dedicadas a mostrar cómo \emph{construir} tipos que pueden codificar los objetos
de nuestras demostraciones.

Una diferencia fundamental entre tipos y conjuntos es que los tipos 
\emph{no contienen} valores, \emph{si no clasifican} valores. En teoría de conjuntos un
valor es un \emph{miembro} de un conjunto, mientras que en teoría de tipos, un
valor \emph{tiene} un tipo. La notación análoga a la de pertenencia en teoría de
conjuntos (\(x \in S\)) en teoría de tipos es el \emph{juicio de tipado} (\emph{typing judgement}), 
que se escribe como \(x: \alpha\), y se lee como \enquote{x tiene tipo
\(\alpha\)}.
Mientras que en teoría de tipos \(x \in S\) y \(x \notin S\) son expresiones
(en el sentido lógico) válidas, el juicio de tipado es \enquote{siempre verdadero}, no es
una proposición que se pueda probar o refutar. Debido a esto no podemos
introducir una variable sin especificar su tipo. Cuando escribimos \(x\) estamos
escribiendo implícitamente \(x: \alpha\), donde \(\alpha\) es el tipo que tendrá \(x\).

Otra importante diferencia es que la teoría de tipos es \emph{su propio sistema
deductivo}, mientras que la teoría de conjuntos está formulada dentro de la
lógica de primer orden. Esto significa que la teoría de conjuntos tiene dos
nociones primitivas, conjuntos y proposiciones, mientras que la teoría de tipos
tiene tipos y funciones. Con solo estas dos podemos recrear toda la lógica de primer
orden, y de hecho, podemos usar tipos para codificar proposiciones, y funciones
para codificar demostraciones. El juicio de tipo \(x: \alpha\) también puede ser leído como
\enquote{x es una demostración de \(\alpha\)} (más en general, de que el tipo es \emph{no vacío}), y el tipo de función \(\alpha \to \beta\) puede ser leído como
\enquote{si tenemos una demostración de \(\alpha\) podemos construir una demostración de
\(\beta\)}. Esto significa que la abstracción de funciones corresponde a una
\emph{implicación lógica} en la lógica clásica, y la aplicación de funciones
corresponde a la \emph{eliminación de implicaciones}. Veremos más sobre esto en la
Sección~\ref{sec:curry_howard}.

\section{Igualdad}

La igualdad en teoría de tipos es ligeramente más compleja que en las matemáticas
clásicas. La igualdad que tenemos \enquote{gratis} es la \emph{igualdad
definicional}, es decir, dos términos son iguales si pueden ser transformados
en otros mediante \emph{reglas de reescritura} (análogas a las reglas de reducción
del \(\lambda\)-cálculo). Las reglas de reducción de las teoría de tipos
típicamente incluyen:
\begin{itemize}
  \item \(\beta\)-reducción. Reemplazar la aplicación de una función por el
    cuerpo de la función, donde el argumento se reemplaza por la variable ligada.
  \item \(\delta\)-reducción. Reemplazar una constante por su definición.
  \item \(\iota\)-reducción. Simplificar expresiones con constructores y encaje
    de patrones para tipos de datos inductivos.
\end{itemize}

Estas reglas satisfacen normalmente un número de propiedades deseables, como
\emph{confluencia}, \emph{terminación}, y \emph{consistencia}, que aseguran que el
proceso de reducción está bien definido, y que la relación de igualdad es una
relación de equivalencia. En algunos casos se añaden otras propiedades como
\emph{congruencia}, \emph{normalización fuerte} y \emph{canonicidad}.

Otro tipo de igualdad es la \emph{igualdad proposicional}, que establece que dos
términos son iguales \emph{si se puede demostrar que son iguales} en la teoría de
tipos. Esta es más parecida a la noción clásica de igualdad, y se usa para
probar propiedades de los términos. Un caso especial de igualdad proposicional es
la \emph{extensionalidad funcional}, que establece que dos funciones son
iguales si son extensionalmente iguales dando resultados iguales sobre cada argumento posible,
un resultado clásico de la teoría de conjuntos que sigue del axioma de extensionalidad, 
que establece que dos conjuntos son iguales si contienen los mismos elementos.

Otro axioma importante es la \emph{extensionalidad proposicional}, que
establece que dos proposiciones son iguales si son equivalentes, es decir, si se
implican mutuamente. Esta es una propiedad clave de la teoría de tipos, ya que
permite reescribir proposiciones en las demostraciones y simplificar el razonamiento.

La igualdad es también una \emph{relación binaria}, es decir, una relación que relaciona
dos valores. En teoría de tipos las relaciones binarias son funciones
que toman dos valores y devuelven una proposición, esta proposición es verdadera
si la relación se cumple, y falsa en caso contrario.

\begin{definition}[Relación binaria]
  Una relación binaria es una función que toma dos elementos y devuelve una
  proposición.
  \[
  \funfont{Rel} \ (\alpha : \texttt{Type}) \ (\beta : \texttt{Type}) 
    \coloneqq \alpha \to \beta \to \Prop 
  \]
\end{definition}

Podemos entonces definir la relación de igualdad como:

\begin{definition}[Relación de igualdad]
  La relación de igualdad es una relación binaria que toma dos elementos de un
  mismo tipo y devuelve una proposición.
  \[
  \funfont{Eq} \ (\alpha: \texttt{Type}) \coloneqq \texttt{Rel} \ \alpha \ \alpha  
  \]
\end{definition}

La igualdad es, sin embargo, mucho más que una relación de equivalencia. Tiene la
importante propiedad de que cada afirmación respeta la equivalencia, en el sentido
de que podemos sustituir expresiones por iguales sin cambiar el valor de verdad.

\subsection{Equivalencia}

La noción de equivalencia es más general que la igualdad, y se refiere a una relación
entre dos tipos que es reflexiva, simétrica y transitiva. Esta relación junto
con el tipo en el que se define conforma un \emph{setoid}, que es análogo 
a los \emph{conjuntos cociente} en teoría de conjuntos. Técnicamente nos define
\emph{isomorfismos} entre tipos.

\section{Universos}

Al principio de la sección hemos introducido el juicio \(x: \alpha\), en el que
\(x\) es un \enquote{valor} y \(\alpha\) es un \enquote{tipo}, pero ahora nos preguntamos
¿cuál es el tipo de \(\alpha\)? La respuesta es que \(\alpha\) es de tipo
\(\tyfont{Type}\), pero ¿cuál es el tipo de \(\tyfont{Type}\)? Por supuesto también es
un tipo, así que \(\tyfont{Type}: \tyfont{Type}\). 
Esto lleva a un problema conocido como \emph{impredicatividad}.

La impredicatividad surge cuando una definición de un objeto o tipo depende de un
cuantificador que abarca una totalidad que incluye el propio objeto que se está
definiendo. Este aspecto autorreferencial puede llevar a paradojas lógicas,
como la paradoja de Russell en teoría de conjuntos, donde un conjunto se define
como aquel que contiene todos los conjuntos que no se contienen a sí mismos.

El análogo a la paradoja de Russell en teoría de tipos es la \emph{paradoja de
Girard}, en un sistema que permite la impredicatividad, podemos definir un tipo \(U\)
que \enquote{contiene} todos los tipos que no se \enquote{contienen} a sí mismos, y luego preguntar si
\(U: U\), si lo es, entonces no debería ser de tipo \(U\), y si no lo es, entonces
debería serlo. Un sistema que muestra esta paradoja no es muy útil para la
demostración de teoremas, ya que es inconsistente, por lo que cualquier
proposición puede ser probada.\footnote{
  Históricamente este problema se ha resuelto con el \emph{Sistema U}.} Para evitar
esto, introducimos una jerarquía de \emph{universos de tipos}.

Cada universo es una \emph{colección de tipos}\footnote{Si estos son tipos o no depende de la
  teoría de tipos en particular, en Lean y Coq no lo son}, cada universo contiene los
tipos del universo anterior, y está contenido en el siguiente universo. Por
ejemplo: \(\tyfont{Type}: \tyfont{Type} \ 1\),
\(\tyfont{Type} \ 1: \tyfont{Type} \ 2\), y así sucesivamente. El tipo función
habita el universo más pequeño que contiene los tipos de su dominio y codominio.
Por ejemplo, \(\mathbb{N} \to \mathbb{N}: \tyfont{Type} \ 1\), y
\(\tyfont{Type} \ 1 \to \tyfont{Type} \ 2: \tyfont{Type} \ 3\).

La base de la jerarquía, el universo \(\tyfont{Type} \ 0\), contiene los tipos del
lenguaje de programación en sí, como \(\mathbb{N}\), \(\tyfont{Bool}\), y
\(\tyfont{List}\). El segundo universo, \(\tyfont{Type} \ 1\), contiene los tipos
del primer universo, y así sucesivamente. Esta jerarquía de universos evita la
impredicatividad, ya que cada universo es un tipo distinto y no puede contenerse
a sí mismo.

\section{Constructores}

Hemos visto varias propiedades de los tipos, pero no hemos visto cómo crear
valores de estos tipos. Por ejemplo, para el tipo \(\tyfont{Bool}\), ¿cómo
podemos crear el valor \(\LeanTrue\)? Para esto necesitamos \emph{constructores de
tipos}, que son funciones que crean elementos de un tipo y definen las
posibles formas que un elemento de un tipo puede tomar. Para \(\tyfont{Bool}\) tenemos dos
constructores, \(\LeanTrue\) y \(\LeanFalse\), que crean los elementos del
tipo. Esto asegura que los elementos del tipo son \emph{solo} \(\LeanTrue\) o
\(\LeanFalse\), y ningún otro valor.

\section{Encaje de patrones}

Para definir funciones que operan sobre tipos necesitamos una forma de distinguir
entre las distintas formas que un elemento de un tipo puede tomar. Esto se
hace usando \emph{encaje de patrones}. El encaje de patrones es una forma de
definir funciones definiendo un caso distinto para cada constructor del tipo. Por
ejemplo, podemos definir una función \(\texttt{not}\) que toma un booleano y
devuelve su negación como:
\[
  \funfont{not} \ \LeanTrue = \LeanFalse \qquad
  \funfont{not} \ \LeanFalse = \LeanTrue
\]

\section{Tipos suma y producto}

Hemos visto que podemos construir tipos a partir de otros mediante funciones. 
Otro mecanismo que tenemos para construir tipos son los \emph{tipos suma} y
\emph{tipos producto}. Estos tipos son construcciones primitivas de la
teoría de tipos, y son usados para construir tipos complejos a partir de tipos
más simples. Los tipos suma son equivalentes a la \emph{unión disjunta} en teoría de
conjuntos, y son usados para representar tipos que pueden tomar uno de varios
valores. Denotamos un tipo suma como \(\alpha + \beta\), donde \(\alpha\) y \(\beta\) son
tipos, los dos constructores de este tipo son \(\funfont{inl}\) y
\(\funfont{inr}\) (abreviaciones de \emph{inyección izquierda} e
  \emph{inyección derecha}), que crean un valor del tipo suma. 
Mediante encaje de patrones podemos definir funciones que operan sobre tipos suma,
distinguiendo entre los dos constructores. 

Cuando hemos definido antes el tipo \(\tyfont{Bool}\) como
el tipo que puede tomar un valor de \(\LeanTrue\) o \(\LeanFalse\), hemos
utilizado implícitamente un tipo suma. En efecto, sea \(\tyfont{Unit}\) el tipo
\emph{unitario} (que tiene un único constructor \texttt{()}), podríamos alternativamente definir 
\(\tyfont{Bool}\) como el \emph{tipo binario} que es la suma de \(\tyfont{Unit}\) y
\(\tyfont{Unit}\):
\[
  \tyfont{Bool} \coloneqq \tyfont{Unit} + \tyfont{Unit}
\]
donde la inyección izquierda \(\funfont{inl}\) representa \(\LeanTrue\) y la
inyección derecha \(\funfont{inr}\) representa \(\LeanFalse\).
Mediante encaje de patrones podríamos definir la función \texttt{not} como:
\begin{align*}
  \funfont{not} \ (\funfont{inl} \ \placeholder) &= \funfont{inr} \ \texttt{()} \\
  \funfont{not} \ (\funfont{inr} \ \placeholder) &= \funfont{inl} \ \texttt{()}
\end{align*}

\begin{notation}
  Utilizamos el símbolo \(\placeholder\) (\emph{placeholder} o \emph{marcador de
  posición} en inglés) para denotar un \emph{agujero} (o \emph{hole}), es decir, un argumento que no
  utilizamos en una expresión, o que puede ser inferido por las reglas de tipado.
\end{notation}

Y definimos la función \texttt{and} como:
\begin{align*}
  \funfont{and} \ (\funfont{inr} \ \placeholder) \ \placeholder &= \funfont{inr} \ \texttt{()} \\
  \funfont{and} \ (\funfont{inl} \ \placeholder) \ y &= y
\end{align*}
Pero, aunque el uso de esta operación es ciertamente interesante, no resulta de momento muy practico,
pues solo podemos definir así tipos finitos. Enseguida veremos una generalización de los 
tipos suma, los \emph{tipos inductivos} que ya acaban con esa limitación.

Los tipos producto son equivalentes al \emph{producto cartesiano} en teoría de
conjuntos, y son usados para representar tipos que pueden tomar varios valores
al mismo tiempo, visto de otra forma \enquote{parejas} de valores. Denotamos un tipo
producto como \(\alpha \times \beta\), donde \(\alpha\) y \(\beta\) son tipos, su
único constructor es \(\funfont{intro}: \alpha \to \beta \to \alpha \times \beta\), 
que crea un valor del tipo producto.
Definimos también las \emph{proyecciones} \(\funfont{fst}\) y
\(\funfont{snd}\), que devuelven el primer y segundo elemento de la pareja
respectivamente.

Podemos definir por ejemplo el tipo \(\tyfont{And}\) (que no se debe confundir
con la función \(\funfont{and}\) de \(\tyfont{Bool}\)) como el tipo producto de dos 
proposiciones\footnote{Veremos el sentido de esto en la sección~\ref{sec:curry_howard}}:
\[
  \tyfont{And} \coloneqq \Prop \times \Prop
\]
De esta forma la demostración izquierda de la conjunción 
\(x : \tyfont{And} \coloneqq (p, q)\) es \(\funfont{fst} \ x = p\)
y la derecha \(\funfont{snd} \ x = q\). La generalización a n-tuplas de
valores es inmediata.

\section{Tipos inductivos}

Definir tipos finitos como \(\tyfont{Bool}\) es interesante, pero no nos
llevaría muy lejos Necesitamos una forma de definir tipos infinitos, como los
números naturales. Esto se hace usando \emph{tipos inductivos}, que son tipos
definidos por un conjunto de constructores que definen las posibles formas que
un elemento del tipo puede tomar. Por ejemplo, podemos definir los números
naturales como un tipo inductivo con dos constructores, \(0\) y
\(\funfont{succ}\), que crean los elementos del tipo, de una forma análoga a como 
lo hacen los \emph{axiomas de Peano}. En este caso los números naturales tienen los
constructores:
\[
  0 : \mathbb{N} \qquad \funfont{succ}: \mathbb{N} \to \mathbb{N}
\]

De forma similar al principio clásico de inducción, hemos definido un caso base
\(0\) y un caso inductivo \(\funfont{succ}\), que crea el sucesor de un número
natural. De esta forma definimos el tipo de los números naturales como un tipo
inductivo, y definir funciones que operan sobre ellos, como la suma,
multiplicación y exponenciación. Esta es la esencia de los tipos inductivos, la
capacidad de definir tipos que son creados por un conjunto de constructores, y
definir funciones que operan sobre ellos. Y como es el caso en la matemática
clásica, podemos usar la inducción para probar propiedades de estos tipos, por 
medio de la \emph{inducción estructural}.

Siguiendo el ejemplo de los números naturales, podemos construir una función
\(f: \mathbb{N} \to C\) por recursión sobre los naturales. Es suficiente con
proveer un caso base \(c_0 : C\) y una función de \enquote{siguiente paso}
\(c_s: \mathbb{N} \to C \to C\), tras lo que definimos \(f\) mediante encaje de
patrones como:
\begin{align*}
  f \ 0 & = c_0 \\
  f \ (\funfont{succ} \ n) & = c_s \ n \ (f \ n)
\end{align*}
Para usar esta técnica debemos asegurar que el tipo inductivo es \emph{bien fundado},
es decir, que los constructores del tipo son aplicados un número finito de veces
para crear un elemento del tipo.

\begin{example}[Suma de naturales]
  Definimos la suma de dos números naturales como:
  \begin{align*}
    \funfont{add} \ n \ 0 &= n \\
    \funfont{add} \ n \ (\funfont{succ} \ m) &= \funfont{succ} \ (\funfont{add} \ n \ m)
  \end{align*}
\end{example}

\section{Tipo función dependiente}

Como dijimos antes, las funciones son ciudadanos de primera clase en la teoría de tipos,
y pueden ser pasadas como argumentos, devueltas como resultados, y \enquote{almacenadas en
variables}. Nos proponemos generalizar el tipo función de forma que el tipo de
retorno dependa del valor de los argumentos. Llamaremos a esta generalización \emph{\(\Pi\)-tipos}
(o \emph{tipos dependientes}).

La notación para un \(\Pi\)-tipo es \(\prod_{x: \alpha} \, B  \ x\), donde \(x\) es el argumento
del cual depende el tipo, \(\alpha\) es el tipo del argumento, y \(B\) es una familia
de tipos indexados por \(x\) (\(B: \alpha \to \tyfont{Type}\)). Por ejemplo, podemos
definir una función que toma un número natural \(n\) y devuelve un tipo
\(\tyfont{Vec} \ \alpha \ n\), que es un tipo de vector de longitud \(n\) de
elementos de tipo \(\alpha\).
\begin{example}[Vector]
  Definimos los vectores de longitud \(n\) por medio de un tipo inductivo que depende de la
  longitud del vector
  \[
  \prod_{n: \mathbb{N}} \tyfont{Vec} \ \alpha \ n
  \]
  que tiene como constructores:
  \begin{align*}
    \funfont{nil} & : \tyfont{Vec} \ \alpha \ 0 \\
    \funfont{cons} & : \prod_{n: \mathbb{N}}
    (\alpha \to \tyfont{Vec} \ \alpha \ n
      \to \tyfont{Vec} \ \alpha \ (\funfont{succ} \ n)) 
  \end{align*}
\end{example}

De esta forma los \(\Pi\)-tipos nos proporcionan una forma de tipar estructuras de
datos complejas, como listas, árboles y grafos, cuyo tipo depende del valor
de sus elementos. 

\section{Tipo par dependiente}

Ahora nos proponemos generalizar los tipos producto de forma que el tipo de
la segunda componente dependa del valor de la primera. Llamaremos a esta
generalización \emph{\(\Sigma\)-tipos} (o \emph{tipos pares dependientes}). La
notación para un \(\Sigma\)-tipo es \(\sum_{x: \alpha} \, B \ x \, \), donde \(x\) es 
la primera componente del par, \(\alpha\) su tipo, y \(B\) es una familia de
tipos indexados por \(x\) (\(B: \alpha \to \tyfont{Type}\,\)).

En caso de que \(B\) sea una función constante, el \(\Sigma\)-tipo es
igual al tipo producto. Es decir, si \(B \ x = \beta\) para todo \(x\), entonces:
\[
  \sum_{x: \alpha} B \ x = \alpha \times \beta.
\]

Su constructor es \(\funfont{intro}: \alpha \to B \ x \to \sum_{x: \alpha} B \ x \, \), que toma un
valor de tipo \(\alpha\) y un valor de tipo \(B \ x\) y devuelve un valor del
tipo \(\sum_{x: \alpha} B \ x \, \).

Aunque la utilidad de estos tipos no es tan evidente como la de los \(\Pi\)-tipos, 
veremos en la siguiente sección que resultan fundamentales para la inclusión de 
las proposiciones como tipos.

\section{La correspondencia de Curry-Howard}\label{sec:curry_howard}

El isomorfismo de Curry-Howard establece una correspondencia entre \emph{proposiciones}
y \emph{tipos}, y entre \emph{términos} y \emph{demostraciones}. Este es conocido como el
\emph{principio de Proposiciones como tipos}, y es la base de todos los
asistentes de prueba modernos, como Coq, Lean y Agda. En nuestro sistema de tipos
cada proposición un tipo es de tipo \(\Prop\). Las \enquote{demostraciones} son términos de
ese tipo, y la proposición es verdadera si el tipo está habitado, es decir, si
hay un término de ese tipo que es \emph{testigo} de su veracidad. 
Consideremos por ejemplo el teorema \enquote{\(\sqrt{2}\) es irracional}. 
Esta proposición vive en el tipo \(\Prop\), y su demostración es un
término de ese tipo, que sabemos que está habitado.

Las demostraciones pueden ser manipuladas como en la lógica clásica. Por ejemplo
podemos obtener una demostración de que dos proposiciones son ciertas usando el
operador \(\land\), que establece que si \(P\) y \(Q\) son verdaderas, entonces
\(P \land Q\) es verdadero. Como hemos visto antes, esto corresponde al \emph{tipo
producto} \(\tyfont{And} \coloneqq P \times Q\, \), que establece que si tenemos una 
demostración de \(P\) y una demostración de \(Q\), entonces podemos construir una
demostración de \(P \times Q\, \). El juicio \(\bot\), que representa la
\emph{inconsistencia}, se representa como un tipo sin constructores \(\tyfont{Empty}\), 
y el juicio \(\top\) que representa una \emph{tautología} se representa como el tipo unitario
\(\tyfont{Unit}\), que tiene solo un constructor sin argumentos.

El principio \emph{ex falso quodlibet} se representa como una función de tipo
\(\funfont{exfalso}: \tyfont{Empty} \to \alpha\), que establece que si tenemos una
demostración de \(\bot\), entonces podemos construir una demostración de cualquier
proposición \(\alpha\). Claramente, si intentamos definir esta función mediante
encaje de patrones tenemos que no podemos discriminar el valor de retorno
dependiendo del valor de entrada, ya que no hay ningún constructor de
\(\tyfont{Empty}\).

Consideramos ahora la implicación (\(\implies\)) en lógica proposicional. La
proposición \(P \implies Q\) establece que si \(P\) es verdadero, entonces \(Q\)
debe ser verdadero. En teoría de tipos esto corresponde al tipo función
\(P \to Q\, \), que establece que si tenemos una demostración de \(P\), entonces
podemos construir una demostración de \(Q\, \). La demostración de la proposición
corresponde a un término del tipo, y la demostración de la implicación
corresponde a una función que toma una demostración de \(P\) y devuelve una
demostración de \(Q\, \).

La correspondencia no se limita a la lógica proposicional, sino que se
extiende a la lógica de predicados y las lógicas de orden superior. Por ejemplo, el
cuantificador universal \(\forall\) corresponde al tipo función dependiente
\(\Pi\, \). En efecto, la proposición \(\forall x: \alpha, P \ x\) corresponde al tipo
\(\prod_{x: \alpha} P \ x\, \), y el cuantificador existencial \(\exists x : \alpha, P \ x\)
corresponde al tipo par dependiente \(\sum_{x: \alpha} P \ x\, \), donde las 
\emph{familias indexadas} \(P: \alpha \to \Prop\) son proposiciones 
que dependen de un valor. De esta forma podemos razonar sobre programas usando las
herramientas de la lógica, y viceversa.

Clarificamos esto con un ejemplo, la desigualdad entre naturales \(n \leq m\) se
puede expresar como la proposición \(n \leq m \coloneqq \exists k: \mathbb{N}, n + k = m\, \).
Para demostrar que \(3 \leq 5\) debemos construir un término del tipo
\(\sum_{k: \mathbb{N}} \, \funfont{add} \ 3 \ k = 5\, \). Para ello usamos el constructor
de \(\Sigma\)-tipos:
\[
  \funfont{intro} \ 2 \ (\funfont{add} \ 3 \ 2 = 5)
\]
Ahora podemos \enquote{evaluar} la función \(\funfont{add}\) obteniendo:
\[
  \funfont{intro} \ 2 \ (5 = 5)
\]
y aplicando el constructor de igualdad \(\funfont{refl}\):
\[
  \funfont{intro} \ 2 \ (\funfont{refl} \ 5)
\]
conseguimos un término del tipo \(\sum_{k: \mathbb{N}}\, \funfont{add} \ 3 \ k = 5\, \), que
es una demostración de \(3 \leq 5\, \). Este ejemplo muestra como realizaremos 
todas las demostraciones en Lean a partir de ahora.

El \emph{verificador de tipos} del sistema nos permite \emph{verificar} las
demostraciones, comprobando que los términos están bien tipados, y que los términos
corresponden a demostraciones válidas. Este mecanismo asegura que las
demostraciones son correctas, y que los programas están bien tipados.

La \enquote{demostración} de que la teoría de tipos y la teoría de conjuntos son
\enquote{igual de potentes} consiste en recrear la teoría de conjuntos en la teoría de
tipos, y viceversa. Esto va más allá de los objetivos de este trabajo, pero
en la siguiente sección veremos como recrear conjuntos dentro
de la teoría de tipos, ya que más adelante los necesitaremos.

\section{Conjuntos}\label{sec:sets}

Podemos codificar un conjunto en teoría de tipos como una función que toma un
valor de un tipo y devuelve una proposición, que nos dice si el
valor está en el conjunto o no.

\begin{definition}[Conjunto]
  Un conjunto es una función que toma un elemento de un tipo y devuelve una
  proposición.
  \[
  \tyfont{Set} \ (\alpha: \tyfont{Type}) \coloneqq \alpha \to \Prop 
  \]
\end{definition}

Podemos usar también la notación tradicional de \emph{generador de conjuntos}.
\begin{notation}[Generador de conjuntos]
  Definimos un conjunto usando la notación de generador de conjuntos como:
  \[
  \{x \mid P\} \coloneqq \lambda x \mapsto P 
  \]
\end{notation}

La definición de la relación de pertenencia es inmediata, ya que la
propia definición del conjunto captura el concepto de pertenencia.
\begin{definition}[Pertenencia]
  La relación de pertenencia es una función que toma un valor de un tipo y un
  conjunto y devuelve una proposición, sea \(x: \alpha\) y \(S: \tyfont{Set} \ \alpha\):
  \[
  x \in S \,\coloneqq\, S \ x 
  \]
\end{definition}

Los subconjuntos se definen como en teoría de conjuntos.
\begin{definition}[Subconjunto]
  Un subconjunto es una función que toma dos conjuntos y devuelve una
  proposición, sea \(A \ B: \tyfont{Set} \ \alpha\):
  \[
  A \subseteq B \coloneqq \forall x, x \in A \implies x \in B 
  \]
\end{definition}

\begin{definition}[Subconjunto estricto]
  Un subconjunto estricto es un subconjunto diferente del conjunto en sí, sea
  \(A \ B: \tyfont{Set} \ \alpha\), entonces:
  \[
  A \subset B \coloneqq A \subseteq B \land A \neq B
  \]
\end{definition}

Con estas definiciones las operaciones conjuntistas se definen de la forma
habitual, y podemos razonar sobre ellas usando las herramientas de la teoría de
tipos.
\begin{definition}[Operaciones conjuntistas]
  Definimos las operaciones conjuntistas como:
  \begin{align*}
    A \cup B & \coloneqq \{x \mid x \in A \lor x \in B\} \\
    A \cap B & \coloneqq \{x \mid x \in A \land x \in B\} \\
    A \setminus B & \coloneqq \{x \mid x \in A \land x \notin B\} \\
    A^\complement & \coloneqq \{x \mid x \notin A\} 
  \end{align*}
\end{definition}

Necesitaremos usar conjuntos cuando hagamos nuestra introducción a la
\emph{Teoría de dominios}.

\section{Irrelevancia de demostraciones}

Una diferencia fundamental entre las diferentes teorías de tipos es cómo
tratan la igualdad entre demostraciones. El sistema de tipos de Lean, como hemos
dicho antes, tiene un universo especial \(\Prop\), en la base de su jerarquía
de tipos, que contiene las proposiciones. En este universo, todas las demostraciones
de la misma proposición son \emph{iguales}.

Esto quiere decir que si tenemos, por ejemplo, dos demostraciones de que \(2 + 2 = 4\), una
que usa los axiomas de Peano, y otra que usa la definición de suma, estas dos
demostraciones son \emph{iguales}, y pueden ser usadas indistintamente.
Más formalmente:
\[
  \forall P: \Prop, \forall p_1, p_2: P\, , \,p_1 = p_2
\]

La ventaja de esto es que nos permite simplificar demostraciones complejas,
ya que no tenemos que preocuparnos por la forma en que se construyen las
demostraciones, sino que podemos tratarlas como \enquote{iguales}. Esto también
ayuda a la hora de \emph{extraer código} de las demostraciones, ya que podemos
\enquote{borrar} las demostraciones en tiempo de compilación y quedarnos solo con 
su conclusión. Podemos, por ejemplo, crear una estructura de datos como una lista,
definir sus operaciones y demostrar sus propiedades, y, una vez que estemos
seguros de su corrección, extraer el código a otro lenguaje, como OCaml,
sin incurrir en el coste en tiempo de ejecución de ejecutar las demostraciones.

El axioma de \emph{irrelevancia} de demostraciones implica otro axioma conocido
como \emph{unicidad de las demostraciones de identidad} o \emph{UIP}.
\begin{definition}[UIP]
  Sean \(x \ y\, : \,\alpha\) y \(p \ q\, : \,x = y\), entonces \(p \,=\, q\).
\end{definition}
Esto se debe a que en Lean \(\tyfont{Eq} \, \ \alpha \, \ x \, \ y\) tiene tipo \(\Prop\), y
por tanto todas las demostraciones de igualdad son iguales.

Como curiosidad, o potencial desventaja de este enfoque, notamos que UIP no es
compatible con el \emph{axioma de univalencia} de la teoría de tipos homotópica.

En Coq y Agda, por el contrario, no tenemos irrelevancia de demostraciones.
Esta es una de las mayores diferencias entre estos demostradores.

\chapter{Semántica}
\epigraph{The most important property of a program is whether it accomplishes the intentions of its user.}{C.A.R. Hoare}

Una vez presentada la teoría de tipos (y los asistentes de demostración) podemos 
pasar a definir la semántica de los lenguajes de programación. Seguiremos un desarrollo
similar al de~\cite{winskel_formal_semantics}. La semántica es el estudio del
\emph{significado} de los programas, y puede ser definida de diferentes formas.
En este trabajo nos centraremos en la semántica operacional y en la semántica
denotacional.

Para nuestro estudio primero definiremos un pequeño lenguaje imperativo
simple que usaremos para ilustrar los conceptos. \(\ImpName\) 
consiste en un conjunto de instrucciones (como asignaciones, secuencias,
condicionales y bucles) y otro de expresiones (aritméticas y booleanas). El lenguaje es
sencillo por diseño, para que podamos centrarnos en introducir las técnicas de
la semántica formal, pero estas mismas técnicas pueden ser extendidas a lenguajes
de programación \enquote{reales}, como C, Java y Python.

\section{Sintaxis}

Como introducción informal a la sintaxis de \(\ImpName\) podemos definir las
\enquote{componentes sintácticas} del lenguaje como:
\begin{itemize}
  \item Enteros \(n \in \mathbb{Z}\).
  \item Booleanos \(b \in \tyfont{Bool} = \{\ImpTrue, \ImpFalse\}\).
  \item Variables \(x \in \tyfont{String}\), que almacenan enteros.
  \item Expresiones aritméticas \(a \in \tyfont{Aexp}\), que pueden ser constantes,
    variables u operaciones aritméticas.
  \item Expresiones booleanas \(b \in \tyfont{Bexp}\), que pueden ser constantes,
    variables u operaciones booleanas.
  \item Instrucciones \(c \in \tyfont{Com}\), que pueden ser asignaciones, condicionales,
    bucles o secuencias de instrucciones.
\end{itemize}

Definimos la sintaxis abstracta de \(\tyfont{Aexp}\), en forma de Backus-Naur:
\[
a \Coloneqq n \ | \ x \ | \ a_1 \ImpPlus a_2 \ | \ a_1 \ImpMinus a_2 \ | \ a_1 \ImpTimes a_2
\]
la sintaxis de \(\tyfont{Bexp}\) como:
\[
b \Coloneqq \ImpTrue \ | \ \ImpFalse \ | \ 
  \ImpNot b \ | \ b_1 \ \ImpAnd \ b_2 \ | \ b_1 \ \ImpOr \ b_2 \ | \ a_1 \ \ImpEq \ a_2 \ | \ a_1 \ \ImpLe \ a_2
\]
y la sintaxis de \(\tyfont{Com}\) como:
\[
c \Coloneqq \ImpSkip \ | \ \ImpAssign{x}{a} \ | \ \ImpConcat{c_1}{c_2} \ | \ \ImpIfElse{b}{c_1}{c_2} \ | \ \ImpWhile{b}{c}
\]

Armados con nuestra teoría de tipos podemos definir estos \enquote{conjuntos sintácticos}
como tipos inductivos. El tipo de expresiones aritméticas puede ser definido como un
tipo inductivo \(\tyfont{Aexp}\) con constructores para cada \enquote{constructo
sintáctico}:
\begin{align*}
  \funfont{val} &\coloneqq \mathbb{Z} \to \tyfont{Aexp} &
  \funfont{var} &\coloneqq \tyfont{String} \to \tyfont{Aexp} \\
  \funfont{add} &\coloneqq \tyfont{Aexp} \to \tyfont{Aexp} \to \tyfont{Aexp} &
  \funfont{sub} &\coloneqq \tyfont{Aexp} \to \tyfont{Aexp} \to \tyfont{Aexp} \\
  \funfont{mul} &\coloneqq \tyfont{Aexp} \to \tyfont{Aexp} \to \tyfont{Aexp}
\end{align*}

Mientras que los primeros dos simplemente \enquote{coaccionan} un valor o una
variable en una expresión, los otros \enquote{construyen} una expresión a partir de
dos expresiones. Los constructores para expresiones booleanas son definidos de
forma similar:
\begin{align*}
  \funfont{true}  &\coloneqq \tyfont{Bexp} &
  \funfont{false} &\coloneqq \tyfont{Bexp} \\
  \funfont{not}   &\coloneqq \tyfont{Bexp} \to \tyfont{Bexp} &
  \funfont{and}   &\coloneqq \tyfont{Bexp} \to \tyfont{Bexp} \to \tyfont{Bexp} \\
  \funfont{or}    &\coloneqq \tyfont{Bexp} \to \tyfont{Bexp} \to \tyfont{Bexp} &
  \funfont{eq}    &\coloneqq \tyfont{Aexp} \to \tyfont{Aexp} \to \tyfont{Bexp} \\
  \funfont{le}    &\coloneqq \tyfont{Aexp} \to \tyfont{Aexp} \to \tyfont{Bexp}
\end{align*}

Y las instrucciones son definidos como:
\begin{align*}
  \ImpSkip &\coloneqq \tyfont{Com} &
  \funfont{assign} &\coloneqq \tyfont{String} \to \tyfont{Aexp} \to \tyfont{Com} \\
  \funfont{seq} &\coloneqq \tyfont{Com} \to \tyfont{Com} \to \tyfont{Com} &
  \funfont{ifElse} &\coloneqq \tyfont{Bexp} \to \tyfont{Com} \to \tyfont{Com} \to \tyfont{Com} \\
  \funfont{whileLoop} &\coloneqq \tyfont{Bexp} \to \tyfont{Com} \to \tyfont{Com}
\end{align*}

Una expresión simple en nuestro lenguaje podría ser:
\[
  \funfont{add} \ (\funfont{var} \ \textit{``x''}) \ (\funfont{val} \ 3)
\]
que corresponde al \enquote{árbol sintáctico abstracto} de la expresión. Para
no tener que lidiar constantemente con ASTs utilizaremos algunos \enquote{abusos de
notación}, que corresponden al \enquote{parseo}, y diremos que \(x + 3\), por ejemplo, 
es una expresión aritmética de \ImpName.
Cuando definamos nuestro lenguaje en Lean debemos tener más cuidado al mezclar
nuestro lenguaje y el meta-lenguaje. Usando algunas facilidades que nos
proporciona podemos hacer un pequeño parser para \ImpName, y usando
clases y coacciones podemos mantener nuestros abusos de notación en el
lenguaje huésped. Todos los detalles están en \url{
github.com/remind-me-later/SemanticsLean/blob/main/Semantics/Imp/Lang.lean}.

Usando estos abusos, que facilitan la claridad, podemos escribir en \(\ImpName\)
un programa que calcule el factorial de un número \(n\), como sigue:
\begin{example}[Factorial en \ImpName]
  El factorial de un número \(n\) puede ser calculado usando un simple bucle:
\begin{minted}{rust}
n = 5; i = 2; res = 1;

while i <= n {
    res = res * i;
    i = i + 1
}
\end{minted}
\end{example}

\section{Estado}

Como hemos visto en la sintaxis, nuestro lenguaje imperativo soporta una
característica que caracteriza a todos los lenguajes imperativos: 
tenemos variables, que \emph{conservan estado}, y asignaciones que \emph{modifican el estado}.
La definición de estado es más sutil de lo que uno podría esperar a primera
vista. Un estado es una función que mapea variables a valores. Lo podríamos
definir como:
\[
\funfont{state} : \tyfont{String} \to \mathbb{Z}
\]

Pero vamos a definirlos de forma más general, mediante \emph{mapas}.
\begin{definition}[Mapa]
  Un mapa es una función que mapea claves (cadenas) a valores:
  \[
  \tyfont{Map} \ (\alpha : \tyfont{Type}) \coloneqq \tyfont{String} \to \alpha
  \]
\end{definition}

Aunque la definición del tipo mapa no es particularmente útil si no tenemos una forma de
relacionar claves y valores. Esto se hace usando la función \funfont{update}.
\begin{definition}[Actualizar]
  La función \funfont{update} toma un mapa, una clave y un valor, y devuelve 
  un nuevo mapa que mapea la clave al valor, y todas las otras claves al mismo
  valor que el mapa original:
  \begin{align*}
  \funfont{update} \ & (m: \tyfont{Map} \ \alpha) \ (k: \tyfont{String}) \ (v: \alpha): \tyfont{Map} \ \alpha \coloneqq \\
                      &\lambda k' \mapsto \funfont{cond} \ (k = k') \ v \ (m \ k')
  \end{align*}
\end{definition}

La función \funfont{cond} no es más que azúcar sintáctico para el encaje de
patrones. Se define como:
\begin{align*}
  \funfont{cond} \ \LeanTrue \ v \ w &\coloneqq v \\
  \funfont{cond} \ \LeanFalse \ v \ w &\coloneqq w
\end{align*}

Podemos usar incluso más azúcar y denotar la función \funfont{cond} como un \emph{if then else}:
\begin{align*}
  \funfont{update} \ & (m: \tyfont{Map} \ \alpha) \ (k: \tyfont{String}) \ (v: \alpha): \tyfont{Map} \ \alpha \coloneqq \\
                      &\lambda k' \mapsto \LeanIfElse{k = k'}{v}{m \ k'}
\end{align*}
que no debe ser confundido con el \texttt{if else} del lenguaje \ImpName.
Podemos introducir también una notación para denotar la \enquote{asignación} de un mapa de
forma más familiar:
\begin{notation}[Actualización de mapa]
  Denotaremos \((\funfont{update} \ m \ k \ v)\) como:
  \[m[k \leftarrow v]\]
\end{notation}

\begin{theorem}
  Si asignamos un valor a una variable, el mapa evalúa la clave a ese valor:
  \[
  (m[k \leftarrow v]) \ k = v
  \]
\end{theorem}

\begin{theorem}
  Si asignamos un valor a una variable, el mapa no evalúa a ese valor para
  otras variables (\(k \neq k'\)).
  \[
  (m[k \leftarrow v]) \ k' = m \ k'
  \]
\end{theorem}

Estos dos teoremas son bastante intuitivos. Veamos un tercero que también
es intuitivo, pero no tan fácil de demostrar. Normalmente asignar un valor a una
variable hace que el ordenador \enquote{olvide} el valor anterior. Para la
función \funfont{update} esto no es cierto por definición, pero podemos
demostrarlo fácilmente.
\begin{theorem}
  Asignar un valor a una variable, y luego asignar otro valor a la misma
  variable hace que el mapa \enquote{olvide} el primer valor:
  \[
  m[k \leftarrow v_2][k \leftarrow v_1] = m[k \leftarrow v_1]
  \]
\end{theorem}

Todas las demostraciones se encuentran en el código, pero vamos a intentar 
explicar un poco en términos más matemáticos, la demostración de este último.
\begin{proof}
  Para demostrar la igualdad de ambas funciones usamos el axioma de
  \emph{extensionalidad funcional}, con lo que demostrar el teorema equivale 
  a demostrar:
  \[
  \forall k' , (m[k \leftarrow v_2][k \leftarrow v_1]) \ k' = (m[k \leftarrow v_1]) \ k'
  \]
  Ahora podemos distinguir dos casos:
  \begin{itemize}
    \item[(\(k = k'\))] 
      En este caso la igualdad se cumple al \enquote{desenrollar} la
      definición de la función \funfont{update}:
      \[
        \LeanIfElse{k = k'}{v_1}{\LeanIfElse{k = k'}{v_2}{m \ k'}} =
        \LeanIfElse{k = k'}{v_1}{m \ k'}
      \]
      Notando que \(k = k'\) es cierto, podemos simplificar la expresión a \(v_1 = v_1\).
    \item[(\(k \neq k'\))] Este caso es análogo.
  \end{itemize}
\end{proof}

Las demostraciones de los siguientes teoremas se realizan de forma parecida,
sin más que usar la extensionalidad funcional y el encaje de patrones.

\begin{theorem}
  El orden de las asignaciones no importa, si asignamos un valor a una
  variable y luego asignamos otro valor a otra variable (\(k_1 \neq k_2\)), el
  resultado es el mismo:
  \[
  m[k_2 \leftarrow v_2][k_1 \leftarrow v_1] = m[k_1 \leftarrow v_1][k_2 \leftarrow v_2]
  \]
\end{theorem}

\begin{theorem}
  Si asignamos a una variable el valor que ya tiene, el mapa no cambia:
  \[
  m[k \leftarrow m \ k] = m
  \]
\end{theorem}

Con estas propiedades demostradas podemos definir nuestro tipo \(\tyfont{State}\).
\begin{definition}[Estado]
  Un estado es un mapa que mapea variables a valores como:
  \[
  \tyfont{State} \coloneqq \tyfont{Map} \ \mathbb{Z}
  \]
\end{definition}

Pero esto nos plantea un problema, ¿qué valor tienen las variables cuando el
programa empieza? En algunos lenguajes de programación populares, como C,
cuando se leen variables antes de ser asignadas tienen un valor aleatorio que
depende de lo que haya en ese momento en la memoria, por lo que es \enquote{arbitrario}.
Podríamos modelar esto definiendo un estado \emph{indefinido}, pero el tipo
\(\tyfont{Map}\) que hemos definido es \emph{total}, es decir, está definido
para todas las claves. Alternativamente podríamos definir un mapa parcial, que es un mapa que
no está definido para todas las claves.
\begin{definition}[Mapa parcial]
  Un mapa parcial es un mapa que no está definido para todas las claves. Los
  definimos como:
  \[
  \tyfont{PMap} \ (\alpha : \tyfont{Type}) \coloneqq \tyfont{String} \to \tyfont{Option} \ \alpha
  \]
\end{definition}

Pero esto complicaría innecesariamente las definiciones y las demostraciones, así que
usaremos la aproximación más sencilla de definir el estado inicial como un
mapa que asigna todas las variables a cero. Por supuesto, asignarles cero es
solo una convención, podríamos asignarles cualquier valor.

\begin{definition}[Estado inicial]
  El estado inicial de un programa \(\ImpName\) es la función constante que
  mapea todas las variables a cero:
  \[
  s_0 \coloneqq \lambda x \mapsto 0
  \]
\end{definition}

De esta forma evaluar el \enquote{contenido} de una \enquote{variable} es tan
sencillo como aplicar el estado a una cadena:
\[
(s_0[k \leftarrow 3][k \leftarrow 4][k \leftarrow 7]) \ k = 7
\]

\section{Semántica de paso largo}

Una vez descrita la \emph{sintaxis} del lenguaje, podemos pasar a definir la
\emph{evaluación} de los diferentes constructos sintácticos, empezando por las
expresiones, que son las estructuras más sencillas.

\subsection{Evaluación de expresiones}

Las expresiones de \(\ImpName\) contienen
variables, por lo que no podemos darles significado sin un estado asociado. Si
consideramos la expresión \(2 + 2\), claramente su valor es \(4\), pero si
tenemos la expresión \(x + 2\) no podemos saber su valor sin conocer el
valor de \(x\). Por ello, la evaluación de expresiones se define sobre una
\emph{configuración}, que es un par de una expresión y un estado:
\(\tyfont{Aexp} \times \tyfont{State}\).

Nuestra evaluación está \emph{dirigida por la sintaxis}, es decir, para cada
\emph{constructo sintáctico} del lenguaje definiremos una regla que nos dice
como evaluarlo. Esta regla será una \emph{relación binaria} entre configuraciones
y valores, la llamaremos \(\funfont{bigStep}\) o \emph{evaluación de paso largo}.

\begin{definition}[Paso largo aritmético]
  La relación de paso largo es una relación binaria entre configuraciones y
  valores que tiene tipo:
  \[
  \funfont{bigStep} : \tyfont{Aexp} \times \tyfont{State} \to \mathbb{Z} \to \Prop
  \]  
\end{definition}

Se conoce como \emph{semántica de paso largo} porque evalúa la expresión a un
valor \enquote{en un solo paso}, desde la configuración directamente al valor.
Introducimos una notación más usada en textos clásicos de semántica formal.
\begin{notation}
  Denotamos la relación de paso largo \(\funfont{bigStep} \ (a, s) \ n\) como:
  \[
  (a, s) \bigstep n
  \]
\end{notation}

Los constructores de la relación son las \emph{reglas de evaluación}. Por ejemplo, el
constructor para una expresión constante es:
\[
  \funfont{val} : (n, \placeholder) \bigstep n
\]

Este constructor relaciona la configuración \((n, \placeholder)\) con el valor entero \(n\), 
donde el \(n\) de la izquierda es una expresión 
constante\footnote{En realidad deberíamos escribir \(\funfont{val} \ n\)
  para denotar que es el primer constructor del tipo \(\tyfont{Aexp}\), pero
  haremos un pequeño abuso de notación y lo omitiremos por brevedad.} y el 
\(\placeholder\) denota cualquier estado. Es importante no confundir el constructor
de la \emph{expresión sintáctica} con el de la \emph{relación de evaluación}, ya 
que aunque tienen el mismo nombre la distinción es clara por el contexto.

El constructor para una expresión variable es similar, pero esta vez necesitamos
\enquote{extraer} el valor de la variable del estado:
\[
  \funfont{var} : (x, s) \bigstep s \ x
\]

Nos planteamos ahora definir los constructores para las operaciones
aritméticas. Como son operaciones \emph{binarias} necesitamos conocer
el valor de la expresión izquierda y derecha para evaluar la operación.
Haremos esto \enquote{exigiendo} la evaluación de las expresiones izquierda y derecha en 
las premisas de la regla, o lo que es lo mismo nuestro constructor 
tomará otros dos constructores del mismo tipo. Por ejemplo el
constructor para la operación suma es:
\[
  \funfont{add} : \frac{(a_1, s) \bigstep n_1 \quad (a_2, s) \bigstep n_2}
   {(a_1 \ImpPlus a_2, s) \bigstep n_1 + n_2}
\]
Esta regla dice que si la configuración \((a_1, s)\) evalúa a \(n_1\) y la
configuración \((a_2, s)\) evalúa a \(n_2\), entonces la configuración
\((a_1 \ImpPlus a_2, s)\) evalúa a \(n_1 + n_2\, \).

Los constructores para la sustracción y la multiplicación son análogos:
\begin{align*}
  \funfont{sub} &: \frac{(a_1, s) \bigstep n_1 \quad (a_2, s) \bigstep n_2}
   {(a_1 \ImpMinus a_2, s) \bigstep n_1 - n_2} \\
  \funfont{mul} &: \frac{(a_1, s) \bigstep n_1 \quad (a_2, s) \bigstep n_2}
   {(a_1 \ImpTimes a_2, s) \bigstep n_1 * n_2}
\end{align*}

Podemos pensar que mientras aplicamos las reglas sintácticas del lenguaje
\enquote{construimos} la expresión a partir del texto, mientras que al aplicar 
las reglas de evaluación \enquote{descomponemos} la expresión hasta su valor.
Veamos un ejemplo de \emph{árbol de evaluación} válido.

\begin{example}[Árbol de evaluación]\label{ex:arithmetic_evaluation}
  Evaluamos el valor de la expresión \((x + 5) + (9 - 7)\) en el estado \(s_0\)
  aplicando las \enquote{reglas} (constructores) de la relación de paso largo,
  los subárboles izquierdo y derecho son:
  \begin{align*}
    L &\coloneqq \funfont{add}: \displaystyle{\frac{
      \funfont{var}: (x, s_0) \bigstep 0
      \quad
      \funfont{val}: (5, s_0) \bigstep 5
    }{(x \ImpPlus 5, s_0) \bigstep 5}} \\
    R &\coloneqq \funfont{sub}: \displaystyle{\frac{
      \funfont{val}: (9, s_0) \bigstep 9
      \quad
      \funfont{val}: (7, s_0) \bigstep 7
    }{(9 \ImpMinus 7, s_0) \bigstep 2}}
  \end{align*}
  Con lo que podemos escribir el árbol de evaluación completo como:
\[
  \funfont{add}: \frac{
    L
    \qquad
    R    
  }
  {((x \ImpPlus 5) \ImpPlus (9 \ImpMinus 7), s_0) \bigstep 7}
\]
\end{example}

Este enfoque tiene una desventaja a nivel computacional y es que en cada
\enquote{paso} de la evaluación hemos tenido que \enquote{elegir} qué regla
aplicar. Puesto que en nuestro caso solo podemos aplicar una regla a cada 
constructo podemos definir una \emph{función de evaluación} en el que las
reglas se apliquen mediante encaje de patrones, y no mediante una relación
binaria. Esta función tomará una expresión y un estado y devolverá un valor.

\begin{definition}[Función de evaluación]
  La función de evaluación toma una expresión y un estado y devuelve un valor,
  tiene tipo:
  \[
  \funfont{eval} : \tyfont{Aexp} \to \tyfont{State} \to \mathbb{Z}
  \]
\end{definition}

Notamos que en vez de usar un producto cartesiano, podemos usar una función
\emph{currificada} (o \emph{parcialmente aplicada}) para definir la función de
evaluación. Esto es posible porque la función de evaluación es \emph{total},
es decir, está definida para todas las expresiones y estados, mediante las 
siguientes reglas:
\begin{align*}
  \funfont{eval} \ (\funfont{val} \ v) \ \placeholder &\coloneqq v \\
  \funfont{eval} \ (\funfont{var} \ x) \ s &\coloneqq s \ x \\
  \funfont{eval} \ (\funfont{add} \ a_1 \ a_2) \ s &\coloneqq \funfont{eval} \ a_1 \ s + \funfont{eval} \ a_2 \ s \\
  \funfont{eval} \ (\funfont{sub} \ a_1 \ a_2) \ s &\coloneqq \funfont{eval} \ a_1 \ s - \funfont{eval} \ a_2 \ s \\
  \funfont{eval} \ (\funfont{mul} \ a_1 \ a_2) \ s &\coloneqq \funfont{eval} \ a_1 \ s * \funfont{eval} \ a_2 \ s
\end{align*}

Introducimos notación para denotar la evaluación de una expresión en un
estado.
\begin{notation}[Evaluación aritmética]
  Denotamos la evaluación de la función \(\funfont{eval} \ a \ s\) como la aplicación de la expresión \(a\)
  a un estado \(s\):
  \[
  a \ s
  \]
\end{notation}

Podemos entonces evaluar el ejemplo~\ref{ex:arithmetic_evaluation} directamente
con la función \(\funfont{eval}\):
\[
  ((x \ImpPlus 5) \ImpPlus (9 \ImpMinus 7)) \ s_0 = 7
\]

La ventaja de este enfoque es evidente, ya que podemos calcular el resultado
directamente, en vez de solo decir si un \enquote{árbol de evaluación} es válido o no.
Todo esto suponiendo que ambas definiciones son equivalentes. Afortunadamente lo son:
\begin{theorem}
  La relación de paso largo y la función \(\funfont{eval}\) son equivalentes, es decir, para todas
  las expresiones \(a\), estados \(s\) y valores \(n\), tenemos:
  \[
  ((\, a \,,\, s,\,) \bigstep n) = (\,a \, \ s \,=\, n)
  \]
\end{theorem}

\begin{proof}
Aplicamos el axioma de \emph{extensionalidad proposicional} para transformar la
igualdad en una doble implicación, y luego probamos cada implicación por separado.

Comenzamos probando la implicación de izquierda a derecha
\((a, s) \bigstep n \implies a \ s = n\, \). Procedemos por inducción
sobre la relación de evaluación, los casos para los constructores \(\funfont{val}\)
y \(\funfont{var}\) son inmediatos. En el caso de \(\funfont{add}\) la
expresión tiene la forma \(a_1 + a_2\), con lo que tenemos que probar que
\[
((a_1 \ImpPlus a_2) \ s) = n_1 + n_2
\]
para algún \(n_1\) y \(n_2\). Por la hipótesis de inducción tenemos que
\(a_1 \ s = n_1\) y \(a_2 \ s = n_2\, \), y así solo tenemos que aplicar la
definición de la función \(\funfont{eval}\) para probar la igualdad.
Los casos para la sustracción y la multiplicación son análogos.

La implicación de derecha a izquierda \((a \ s = n) \implies (a, s) \bigstep n\)
se demuestra por inducción sobre la expresión aritmética; los casos para
\(\funfont{val}\) y \(\funfont{var}\) son inmediatos, así que nos enfocamos en
\(\funfont{add}\). Como antes, tenemos que probar que
\((a_1 \ImpPlus a_2, s) \bigstep n\), contando con las hipótesis de inducción:
\[
\begin{gathered}
  \forall n, a_1 \ s = n \implies (a_1, s) \bigstep n \qquad
  \forall n, a_2 \ s = n \implies (a_2, s) \bigstep n
\end{gathered}
\]
Si elegimos como \(n\) en ambos casos \(a_1 \ s\) y \(a_2 \ s\) respectivamente, podemos 
construir el árbol de evaluación:
\[
  \funfont{add}: \frac{
    (a_1, s) \bigstep a_1 \ s
    \quad
    (a_2, s) \bigstep a_2 \ s
  }
  {
   (a_1 \ImpPlus a_2, s) \bigstep a_1 \ s + a_2 \ s
  }
\]
Con lo que basta aplicar la hipótesis \((a_1 \ImpPlus a_2) \ s = n\) para obtener el resultado. Los casos para
la sustracción y la multiplicación son análogos.
\end{proof}

Presentamos las reglas de evaluación de expresiones booleanas sin mucho detalle,
ya que son análogas a las expresiones aritméticas, las cuales hemos presentado
ya puesto que algunas de las reglas para expresiones booleanas dependen de la
evaluación de expresiones aritméticas, (\(\ImpLe\)) y (\(\ImpEq\)):
\[
\begin{gathered}
  \funfont{true}: (\ImpTrue, \placeholder) \bigstep \LeanTrue \qquad
  \funfont{false}: (\ImpFalse, \placeholder) \bigstep \LeanFalse \\
  \funfont{not}: \frac{(b, s) \bigstep b'}{(\ImpNot b, s) \bigstep \lnot b'} \\
  \funfont{and}: \frac{(b_1, s) \bigstep b_1' \quad (b_2, s) \bigstep b_2'}
    {(b_1 \ \ImpAnd \ b_2, s) \bigstep b_1' \land b_2'} \qquad
  \funfont{or}: \frac{(b_1, s) \bigstep b_1' \quad (b_2, s) \bigstep b_2'}
    {(b_1 \ \ImpOr \ b_2, s) \bigstep b_1' \lor b_2'} \\
  \funfont{eq}: (a_1 \ \ImpEq \ a_2, s) \bigstep a_1 \ s = a_2 \ s \qquad
  \funfont{le}: (a_1 \ \ImpLe \ a_2, s) \bigstep a_1 \ s \leq a_2 \ s
\end{gathered}
\]

Notamos que en las últimas dos reglas estamos usando la función
(\funfont{eval}) para evaluar las expresiones aritméticas. Podríamos haber
usado la relación de paso largo, pero este enfoque es más sencillo, y como hemos
visto antes equivalente. Todo lo que hemos dicho sobre la función
(\funfont{eval}) y su equivalencia con la relación de paso largo se aplica también 
a las expresiones booleanas.

\subsection{Evaluación de instrucciones}

Hemos visto en la sección anterior que lo más cómodo para definir la semántica de
expresiones aritméticas y booleanas es usar la función de evaluación
(\funfont{eval}). Vamos a intentar hacer lo mismo con las instrucciones. Las reglas
de nuestra función de evaluación serían algo como:
\begin{align*}
  \funfont{eval} \ \ImpSkip \ s &\coloneqq s \\
  \funfont{eval} \ \ImpAssign{x}{a} \ s &\coloneqq s[x \leftarrow a \ s] \\
  \funfont{eval} \ (\ImpConcat{c_1}{c_2}) \ s &\coloneqq \funfont{eval} \ c_2 \ (\funfont{eval} \ c_1 \ s) \\
  \funfont{eval} \ (\ImpIfElse{b}{c_1}{c_2}) \ s &\coloneqq
    \LeanIfElse{b \ s}{\funfont{eval} \ c_1 \ s}{\funfont{eval} \ c_2 \ s} \\
  \funfont{eval} \ (\ImpWhile{b}{c}) \ s &\coloneqq
    \LeanIfElse{b \ s}{\funfont{eval} \ (\ImpWhile{b}{c}) \ (\funfont{eval} \ c \ s)}{s}
\end{align*}
El problema de esta definición es que no es en realidad la de una función, ya 
que no es \emph{bien fundada}. Si consideramos el bucle \(\ImpWhile{\ImpTrue}{\ImpSkip}\),
claramente aplicando la última regla de la función de evaluación nunca
obtenemos un resultado, ya que la condición es siempre verdadera\footnote{
  Demostramos esto rigurosamente en~\ref{sec:non_termination}.}.
Por tanto deberemos recurrir nuevamente al paso largo. Más tarde veremos como
podemos definir una \enquote{función} que evalúe las instrucciones, que es lo que se
conoce como \emph{semántica denotacional}.

Definimos como antes la semántica de paso largo de las instrucciones como una relación,
esta vez asociando configuraciones a estados. Damos primero la definición de los 
constructores más sencillos:
\begin{align*}
  \ImpSkip &: (\ImpSkip, s) \bigstep s \\
  \funfont{ass} &: (v = a, s) \bigstep s[v \leftarrow a \ s] \\
  \funfont{cat} &: \frac{
    (c_1, s_1) \bigstep s_2 \quad (c_2, s_2) \bigstep s_3
  }{
    (\ImpConcat{c_1}{c_2}, s_1) \bigstep s_3
  }
\end{align*}
En la concatenación introducimos un estado intermedio \(s_2\) que no 
aparece en el resultado final. De esta forma tenemos en cuenta los 
\enquote{pasos intermedios} en la evaluación del lado izquierdo.

Las reglas para la estructura \texttt{if} requieren dos constructores,
uno para el caso en que la condición es falsa:
\[
\funfont{ifFalse} : \frac{b \ s_1 = \LeanFalse \quad (c_2, s_1) \bigstep s_2}
{(\ImpIfElse{b}{c_1}{c_2}, s_1) \bigstep s_2}
\]
y otro para cuando la condición es cierta:
\[
\funfont{ifTrue} : \frac{b \ s_1 = \LeanTrue \quad (c_1, s_1) \bigstep s_2}
{(\ImpIfElse{b}{c_1}{c_2}, s_1) \bigstep s_2}
\]
La regla de evaluación para \texttt{while} sigue el mismo principio, pero con un enfoque más
recursivo. Comenzamos con el caso en que la condición es falsa, que es el más sencillo:
\[
\funfont{whileFalse} : \frac{b \ s = \LeanFalse}
  {(\ImpWhile{b}{c}, s) \bigstep s}
\]
Pero cuando la condición es cierta, tenemos que hacer una recursión desde otro árbol de evaluación
donde el \enquote{paso anterior} ya ha sido aplicado:
\[
\funfont{whileTrue} : \frac{b \ s_1 = \LeanTrue \quad (c, s_1) \bigstep s_2 \quad
  (\ImpWhile{b}{c}, s_2) \bigstep s_3}
{(\ImpWhile{b}{c}, s_1) \bigstep s_3}
\]
Mientras que las reglas de paso largo para expresiones pueden ser aplicadas
\enquote{automáticamente}, las reglas de paso largo para instrucciones requieren un enfoque
más \enquote{manual}, ya que tenemos que \enquote{elegir} el constructor correcto
en el flujo de control del programa.

Con los abusos de notación habituales, podemos comprobar la evaluación de un
programa como este:
\begin{example}[Árbol de evaluación de un programa]
  Evaluamos un programa sencillo, que contiene una bifurcación, en el estado \(s_0\, \):
\begin{minted}{rust}
x = 2;
if x <= 1 {
  y = 3
} else {
  z = 4
}
\end{minted}
  Para aligerar la notación denotaremos el programa como \(P\), y la estructura 
  \texttt{if} como \(\text{Ifelse}\), entonces:
  \[
    F \coloneqq \funfont{ifFalse}: \displaystyle{\frac{
      \funfont{ass}: (\ImpAssign{z}{4} , s_0[x \leftarrow 2]) \bigstep s_0[x \leftarrow 2][z \leftarrow 4]
      }
    {
      (\text{Ifelse}, s_0[x \leftarrow 2]) \bigstep s_0[x \leftarrow 2][z \leftarrow 4]
    }}
  \]
  y el árbol completo es:
  \[
  \funfont{cat}: \frac{
    \funfont{ass}: (\ImpAssign{x}{2}, s_0) \bigstep s_0[x \leftarrow 2]
    \qquad
    F
  }{
    (P, s_0) \bigstep s_0[x \leftarrow 2][z \leftarrow 4]
  }
  \]
\end{example}
Como podemos observar, la \enquote{demostración} de que el programa termina
en un estado dado se sigue directamente del árbol de evaluación creado por los
constructores de la semántica de paso largo.

\subsection{Reglas de reescritura}

Presentamos ahora algunas \enquote{reglas de reescritura} para los constructos del
lenguaje que nos permitirán simplificar las demostraciones. Solamente enunciaremos
los teoremas, cuyas demostraciones pueden encontrarse en el código, y en realidad no
tienen mucho interés.
\begin{theorem}
  Usar la instrucción \ImpSkip{} (no hacer nada) no cambia el estado:
  \[
  ((\ImpSkip, s_1) \bigstep s_2) = (s_1 = s_2)
  \]
\end{theorem}

\begin{theorem}
  Concatenar dos programas es equivalente a ejecutar el primero y luego el segundo:
  \[
  ((\ImpConcat{c_1}{c_2}, s_1) \bigstep s_3) = 
  \exists s_2, \left((c_1, s_1) \bigstep s_2 \land (c_2, s_2) \bigstep s_3\right)
  \]
\end{theorem}

Las reglas de reescritura para el constructo \texttt{if} pueden definirse de
dos formas equivalentes. Qué regla aplicar en cada caso dependerá del contexto, ya que
una u otra podría ser la más fácil de usar dependiendo de la situación.
\begin{theorem}\label{thm:if_rewrite_1}
  El constructo \texttt{if} puede reescribirse como:
  \[
  ((\ImpIfElse{b}{c_1}{c_2}, s_1) \bigstep s_2) = 
  \LeanIfElse{b \ s_1}{(c_1, s_1) \bigstep s_2}{(c_2, s_1) \bigstep s_2}
  \]
\end{theorem}
O alternativamente:
\begin{theorem}\label{thm:if_rewrite_2}
  El constructo \texttt{if} puede reescribirse como:
  \[
  ((\ImpIfElse{b}{c_1}{c_2}, s_1) \bigstep s_2) =
  (\LeanIfElse{b \ s_1}{c_1}{c_2}, s_1) \bigstep s_2
  \]
\end{theorem}

En los teoremas anteriores tenemos que tener cuidado de no confundir el
\enquote{if} del lenguaje \ImpName{} y el del meta-lenguaje (en este caso Lean).
Para ayudar en esta distinción usaremos una fuente \texttt{mono espaciada} para los
constructos del lenguaje, y una fuente normal para los constructos del
meta-lenguaje.

\begin{theorem}\label{thm:while_rewrite}
  El constructo \texttt{while} puede reescribirse como:
  \begin{align*}
  &((\ImpWhile{b}{c}, s_1) \bigstep s_3) = \\
  &\quad \text{if} \ b \ s_1 \\
  &\quad \text{then} \ \exists s_2, ((c, s_1) \bigstep s_2) \land ((\ImpWhile{b}{c}, s_2) \bigstep s_3) \\
  &\quad \text{else} \ s_1 = s_3
  \end{align*}
\end{theorem}

\subsection{Equivalencia de comportamiento}

Las reglas de reescritura anteriores son útiles en las demostraciones, ya que siempre
podemos sustituir un programa por otro que sea \enquote{exactamente el mismo}, pero
también podemos considerar una noción de igualdad más laxa que nos permita decir que
dos programas sintácticamente diferentes son \enquote{el mismo}. Esto es
especialmente útil cuando queremos demostrar que ciertas optimizaciones equivalentes no
cambian el comportamiento de un programa.

\begin{definition}[Equivalencia de comportamiento]
  Dos programas se comportan igual si para todos los estados iniciales:
  \[
  c_1 \sim c_2 \coloneqq \forall s_1 \ s_2, (c_1, s_1) \bigstep s_2 \iff (c_2, s_1) \bigstep s_2
  \]
\end{definition}

Es fácil ver que esta relación es una relación de equivalencia, es decir, es
reflexiva, simétrica y transitiva. Utilizando esta equivalencia podemos, por ejemplo, demostrar que
agregando un número arbitrario de \ImpSkip(s) a un programa nunca cambia su
comportamiento.
\begin{theorem}
  Agregando \ImpSkip{} a la izquierda de un programa no cambia su comportamiento:
  \[
  \ImpConcat{\ImpSkip}{c} \sim c
  \]
\end{theorem}

Ahora podríamos \enquote{optimizar} un programa suprimiendo un skip, pero solo cuando aparece
a la izquierda. Para eliminarlo completamente sin consecuencias necesitamos el
teorema simétrico.
\begin{theorem}
  Agregando \ImpSkip{} a la derecha de un programa no cambia su comportamiento:
  \[
  \ImpConcat{c}{\ImpSkip} \sim c
  \]
\end{theorem}

Una equivalencia más interesante que refleja el teorema~\ref{thm:while_rewrite} es
el \emph{desenrollado de bucles}, una optimización común en compiladores, usada
cuando la memoria es más barata que la computación.
\begin{theorem}
  La instrucción \texttt{while} puede desenrollarse:
  \[
  \ImpWhile{b}{c} \sim 
  \ImpIfElse{b}{\ImpConcat{c}{\ImpWhile{b}{c}}}{\ImpSkip}
  \]
\end{theorem}

\begin{proof}
  Por la definición de equivalencia el problema se reduce a:
  \[
    \forall s_1 \ s_3, (\ImpWhile{b}{c}, s_1) \bigstep s_3 \iff
    (\ImpIfElse{b}{\ImpConcat{c}{\ImpWhile{b}{c}}}{\ImpSkip}, s_1) \bigstep s_3
  \]
  Podemos aplicar el teorema~\ref{thm:if_rewrite_2} al lado derecho de la
  equivalencia:
  \[
    (\ImpWhile{b}{c}, s_1) \bigstep s_3 \iff
    (\LeanIfElse{b \ s_1}{\ImpConcat{c}{\ImpWhile{b}{c}}}{\ImpSkip}, s_1) \bigstep s_3
  \]
  Y ahora basta con probar ambas implicaciones por separado.
  \begin{itemize}
    \item[(\(\implies\))] 
    Comprobamos ambos casos para el constructor \texttt{while} en el lado izquierdo de la implicación.
    \begin{itemize}
      \item[(\(b \ s_1\))]
      Si la condición es cierta, el problema se simplifica a:
      \[
        \ImpConcat{c}{\ImpWhile{b}{c}}, s_1 \bigstep s_3
      \]
      y como sabemos que la precondición es cierta podemos aplicar el constructor
      \(\funfont{whileTrue}\) para obtener el árbol de derivación del lado izquierdo de la equivalencia:
      \[
        \funfont{whileTrue}: \frac{
          b \ s_1 = \LeanTrue
          \quad
          (c, s_1) \bigstep s_2 
          \quad 
          (\ImpWhile{b}{c}, s_2) \bigstep s_3
        }{
          (\ImpWhile{b}{c}, s_1) \bigstep s_3
        }
      \]
      Basta con aplicar la nueva hipótesis de \(\funfont{whileTrue}\) para
      obtener el resultado:
      \[
        \funfont{cat}: \frac{
          (c, s_1) \bigstep s_2
          \quad
          (\ImpWhile{b}{c}, s_2) \bigstep s_3
        }{
          (\ImpConcat{c}{\ImpWhile{b}{c}}, s_1) \bigstep s_3
        }
      \]
      \item[(\(\lnot b \ s_1\))]
      El problema se reduce a:
      \[
        (\ImpSkip, s_1) \bigstep s_3
      \]
      procedemos como antes y aplicamos el constructor \(\funfont{whileFalse}\):
      \[
        \funfont{whileFalse}: \frac{
          b \ s_1 = \LeanFalse         
        }{
          (\ImpWhile{b}{c}, s_1) \bigstep s_1
        }
      \]
      con lo que tenemos que \(s_1 = s_3\) y hemos terminado.
    \end{itemize}

    \item[(\(\impliedby\))]
    Esta dirección se demuestra también por casos sobre la condición
    de la instrucción \texttt{if}, pero es aún más sencilla que antes, ya que podemos
    simplificar la precondición directamente.
  \end{itemize}
\end{proof}

\subsection{No terminación}\label{sec:non_termination}

Como dijimos antes cuando intentamos definir una función \funfont{eval} para
evaluar instrucciones, existen programas de \ImpName{} que no terminan, o lo que
es lo mismo en términos de la relación de paso largo, no existe ninguna 
relación que sigue el programa en el estado dado con otro estado \enquote{final}.

\begin{definition}[No terminación]
  Decimos que un programa no termina en un estado \(s\) si no existe ningún
  estado \(s'\) tal que el programa pueda evaluarse a \(s'\), es decir:
  \[
   \forall s', (c, s) \centernot \bigstep s'
  \]
\end{definition}

Ahora demostramos que existen programas en \ImpName{} que no terminan. Para
ello elegimos un programa que no termina nunca, sin importar el estado
en el que se ejecute.

\begin{theorem}\label{thm:non_termination}
  El programa \(\ImpWhile{b}{c}\), donde \(b \sim \ImpTrue\) no termina,
  independientemente del estado inicial:
  \[
  \forall s_1 \ s_2, (\ImpWhile{b}{c}, s_1) \centernot \bigstep s_2
  \]
\end{theorem}

\begin{proof}
  En realidad el \(\centernot \bigstep\) es azúcar sintáctico para
  la negación de la relación de paso largo, así que el teorema dice en realidad:
  \[
  (\ImpWhile{b}{c}, s_1) \bigstep s_2 \implies \bot
  \]
  Ahora tenemos que usar un pequeño truco. Primero generalizamos la configuración
  a la izquierda, llamándola \text{hconf}:
  \[\text{hconf} \coloneqq (\ImpWhile{b}{c}, s_1)\] 
  con lo que podemos reescribir el teorema como:
  \[
  \text{hconf} \bigstep s_2 \implies \bot
  \]
  Ahora aplicamos el principio de inducción sobre la configuración, es decir,
  demostramos el teorema para todas las formas posibles de \(\text{hconf}\).
  Los únicos casos que nos interesan son los de \(\funfont{whileTrue}\) y
  \(\funfont{whileFalse}\), ya que para el resto de los constructores la
  igualdad es falsa por definición, por ejemplo \(\text{hconf} = (\ImpSkip, s_1)\).

  Para el caso \(\funfont{whileTrue}\) tenemos que probar que:
  \[
    (\ImpWhile{b}{c}, s_1) = (\ImpWhile{b}{c}, s_1')
  \]
  La hipótesis de inducción toma la forma:
  \[
    \forall x, (\ImpWhile{b}{c}, x) = (\ImpWhile{b'}{c'}, s_2')
  \]
  así que basta elegir \(x = s_2\) para obtener el resultado. 
  
  Para el caso \(\funfont{whileFalse}\) no necesitamos usar inducción, de hecho si
  \(\text{hconf}\) toma esta forma significa que la condición del \texttt{while} es
  falsa, pero \(b \sim \ImpTrue\) por hipótesis, así que tendríamos una
  contradicción.
\end{proof}

\subsection{Determinismo}

Otra propiedad importante de la semántica de \ImpName{} de paso largo
es que la evaluación de un programa es determinista. Es decir, dado un
programa y un estado solo a lo sumo hay un estado al que el programa puede evaluarse.

\begin{definition}[Determinismo]\label{thm:deterministic}
  Diremos que un lenguaje es \emph{determinista} si se cumple:
  \[
  \forall c \ s \ s_1 \ s_2, ((c, s) \bigstep s_1 \land (c, s) \bigstep s_2) \implies s_1 = s_2
  \]
\end{definition}

\begin{theorem}
  El lenguaje \ImpName{} es determinista.
\end{theorem}

\begin{proof}
  Haremos la demostración por inducción sobre la relación de paso largo
  \((c, s) \bigstep s_1\, \). Los casos para \ImpSkip{} y la asignación son
  inmediatos, así que pasamos al caso de la concatenación. Tenemos 
  las hipótesis de inducción:
  \begin{align*}
    \forall x, (c_1', s_1') &\bigstep x \implies s_3 = x \\
    \forall y, (c_2', s_2') &\bigstep y \implies s_1 = y
  \end{align*}
  El lado derecho de la conjunción toma la forma
  \((\ImpConcat{c_1'}{c_2'}, s_1') \bigstep s_2\, \), así que tenemos que tener
  sus precondiciones, es decir, necesariamente:
  \[\funfont{ass}: \frac{
      (c_1', s_1') \bigstep s_2' 
      \quad 
      (c_2', s_2') \bigstep s_2
    }{
      (\ImpConcat{c_1'}{c_2'}, s_1') \bigstep s_2
    }
  \]
  Ahora aplicamos la primera hipótesis de inducción en la precondición
  izquierda, obteniendo \(s_3 = s_2'\), con lo que la segunda precondición
  se transforma en \((c_2', s_3) \bigstep s_2\, \). Y aplicando la
  segunda hipótesis de inducción obtenemos el resultado \(s_1 = s_2\).

  El resto de los casos son análogos.
\end{proof}

\section{Semántica estructural}

Otra forma de definir la semántica del lenguaje es la semántica \emph{estructural}
o \emph{de paso corto}. En este caso en lugar de obtener directamente el valor
de la expresión formalizan los pasos que relaciona dos configuraciones:
a la izquierda tenemos el estado antes de realizar el paso, y a la derecha el
estado después de ello, de este modo podemos observar los \emph{estados intermedios} 
de la ejecución de un programa, siendo cada paso de esta semántica 
\enquote{atómico}. Esto resultaría imprescindible para definir la semántica de lenguajes con construcciones 
paralelas o concurrentes, aunque \ImpName{} por el momento no lo sea.
\begin{definition}[Semantica estructural]
  La semántica estructural de un programa viene dada por una relación binaria 
  \(\text{SmallStep} : \tyfont{Com} \times \tyfont{State} \to \tyfont{Com} \times \tyfont{State} \to \Prop\),
  que denotamos por:
  \[
    (c_1, s_1) \rightsquigarrow (c_2, s_2) \, \coloneq \, \text{SmallStep} \ (c_1, s_1) \ (c_2, s_2)
  \]
\end{definition}

En el caso de \ImpName{} los constructores de la relación son:
\[
\begin{gathered}
  \funfont{ass}: (\ImpAssign{v}{a}, s) \rightsquigarrow (\ImpSkip, s[v \leftarrow s]) \\
  \funfont{skipCat}: (\ImpConcat{\ImpSkip}{c}, s) \rightsquigarrow (c, s) \qquad
  \funfont{cat}: \frac{(c, s) \rightsquigarrow (c', s')}{(\ImpConcat{c}{c''}, s) \rightsquigarrow (c', c'', s')} \\
  \funfont{ifElse}: (\ImpIfElse{b}{c}{c'}, s)
    \rightsquigarrow (\LeanIfElse{b \ s}{c}{c'}, s) \\
  \funfont{whileLoop}: (\ImpWhile{b}{c}, s)
    \rightsquigarrow (\LeanIfElse{b \ s}{\ImpConcat{c}{\ImpWhile{b}{c}}}{\ImpSkip}, s) 
\end{gathered}
\]

Estas reglas capturan la \enquote{ejecución} de las instrucciones de izquierda a derecha
en una forma secuencial, simulando de cerca la ejecución de un programa
en un computador. Por supuesto, podríamos haber definido las reglas del 
\enquote{if} y \enquote{while} de otras formas, dependiendo de la \enquote{granularidad}
que queramos en la semántica. Por ejemplo, evaluar la condición podría superar un paso y seleccionar la rama
otro.

Veamos un ejemplo de como funciona la semántica estructural.
\begin{example}\label{ex:small_step}
  Consideramos el programa \texttt{x = 1; while x <= 1 \{x = x + 1\}},
  donde para aligerar la notación denotaremos el bloque \enquote{while} como \(W\).
  El árbol de evaluación de este programa es:
  \[
  \funfont{cat}: \frac{
    \funfont{ass}: (\ImpAssign{x}{1}, s_0) \rightsquigarrow (\ImpSkip, s_0[x \leftarrow 1])
  }
  {(\ImpConcat{\ImpAssign{x}{1}}{W}, s_0)
  \rightsquigarrow
  (\ImpConcat{\ImpSkip}{W}, s_0[x \leftarrow 1])
  }
  \]
\end{example}

En este caso la ejecución termina, ya que no podemos aplicar más reglas al
programa, esto es, tenemos una configuración de la forma \((\ImpSkip, \placeholder)\).
Definiremos la ejecución de un programa de principio a fin con la semántica 
estructural mediante su \emph{clausura reflexiva transitiva}.

\subsection{Clausura reflexiva transitiva}

La clausura reflexiva transitiva es una relación binaria que se crea a partir de
otra y nos da la \enquote{extensión mínima reflexiva y transitiva} de la misma.
Por tanto si la relación original ya es reflexiva y transitiva, la
clausura es la misma relación. Formalmente la definimos como una función que
toma una relación binaria y devuelve otra.

\begin{definition}[Clausura reflexiva transitiva]
  La clausura reflexiva transitiva de una relación binaria \(\mathcal{R}\) es la
  relación más pequeña que contiene \(\mathcal{R}\) y es tanto reflexiva como
  transitiva. En concreto es un tipo inductivo 
  \(\tyfont{ReflTrans}: (\alpha \to \alpha \to \Prop) \to
  (\alpha \to \alpha \to \Prop)\) y la denotamos como
  \(\mathcal{R}^* \coloneqq \tyfont{ReflTrans} \ \mathcal{R}\).

  El tipo \(\tyfont{ReflTrans}\) tiene dos constructores:
  \[
    \funfont{refl}: a \mathcal{R}^* a
    \qquad 
    \funfont{tail}: \frac{
      a \mathcal{R}^* b
      \quad b \mathcal{R} c
    }{a \mathcal{R}^* c}
  \]
\end{definition}

Esta relación no es transitiva de forma inmediata por definición, pero ello es fácil de probar por
inducción.

\begin{theorem}
  La clausura reflexiva transitiva es transitiva. Es decir, si tenemos
  \(x \mathcal{R}^* y\) y \(y \mathcal{R}^* z\) entonces
  \(x \mathcal{R}^* z\).
\end{theorem}

Otra propiedad aún más inmediata, pero importante también, es que la relación de partida
está contenida en su clausura.

\begin{theorem}
  La relación \(\mathcal{R}\) está contenida en su clausura reflexiva transitiva,
  \(a \mathcal{R} b \implies x \mathcal{R}^* y\).
\end{theorem}

Además, no solo podemos \enquote{apilar} en la \enquote{cola} de la relación, sino
que también podemos \enquote{empujar} en la \enquote{cabeza} de la relación.

\begin{theorem}
  Podemos \enquote{empujar} una relación a la cabeza de su clausura reflexiva
  transitiva, es decir, si tenemos \(a \mathcal{R} b\) y \(b \mathcal{R}^* c\) entonces
  \(a \mathcal{R}^* c\).
\end{theorem}

Esta propiedad nos será de gran utilidad, al permitirnos definir un
principio de inducción sobre la \emph{cabeza} de la relación, pues en principio, el uso del 
constructor solo nos permite hacer inducción sobre la \emph{cola}.

\begin{theorem}
  Podemos utilizar el principio de inducción sobre la \emph{cabeza} de la relación, 
  siendo el caso base la reflexividad, y el paso inductivo la transitividad sobre la
  \emph{cabeza} de la relación.\footnote{
    La definición general de lo que significa tener un principio de inducción es muy técnica, 
    por lo que evitaremos incluirla aquí. 
    Pero puede encontrarse en el teorema \texttt{head\_induction\_on} de~\ref{sec:code_refltransgen}.}
\end{theorem}

Otro teorema importante que usamos en el código es la \emph{elevación} de una
clausura reflexiva transitiva a través de una función \enquote{monótona}\footnote{
  En~\ref{sec:order_homomorphisms} definiremos el concepto de monotonía.}.

\begin{theorem}\label{thm:rel_lift}
  Podemos elevar una relación reflexiva transitiva \(x \mathcal{R}^* y\) a través de una función
  \(f: \alpha \to \beta\), siempre que una relación implique a la otra:
  \[h: \forall a \ b, a \mathcal{R} b \to (f \ a) \mathcal{P} (f \ b),\]
  entonces tenemos \((f \ a) \mathcal{P}^* (f \ b)\).
\end{theorem}

\begin{proof}
  Procedemos por inducción sobre la relación \(a \mathcal{R}^* b\), el caso base
  \(\funfont{refl}: a \mathcal{R}^* a\) implica que \(a = b\), así que
  \((f \ a) \mathcal{P}^* (f \ a)\)
  lo que es inmediato por la reflexividad de la relación. Para el paso inductivo
  \[
    \funfont{tail}: \frac{
      a \mathcal{R}^* b 
      \quad 
      b \mathcal{R} c
    }{a \mathcal{R}^* c}
  \]
  tenemos que demostrar que \((f \ a) \mathcal{P}^* (f \ c)\).
  Denotando con \(\text{ih}\) la hipótesis de inducción y aplicando el
  constructor de \texttt{tail} obtenemos el resultado:
  \[
    \funfont{tail}: \frac{
      \text{ih}: (f \ a) \mathcal{P}^* (f \ b)
      \quad
      h: \displaystyle{\frac{
        b \mathcal{R} c
      }{
        (f \ b) \mathcal{P} (f \ c)
      }}
    }{
      (f \ a) \mathcal{P}^* (f \ c)
    } \qedhere
  \]
\end{proof}

\subsection{Semántica estructural natural}

Con lo anterior y la semántica estructural podemos definir una
relación entre cualesquiera dos estados intermedios de la ejecución de un programa,
esta relación es la clausura reflexiva transitiva de la relación de paso corto.

\begin{notation}
  Denotamos la clausura reflexiva transitiva de la semántica de paso corto como
  \(\rightsquigarrow^*\).
\end{notation}

Podemos ahora revisar el ejemplo~\ref{ex:small_step} aplicando
\(\rightsquigarrow^*\), esta vez dando \enquote{varios pasos}.

\begin{example}
  Como antes, consideramos el programa \texttt{x = 1; while x <= 1 \{x = x + 1\}},
  para facilitar la lectura denotaremos el bloque \enquote{while} como \(W\).
  Un árbol de derivación parcial es:
  \[
    H \coloneqq
    \funfont{head}: \displaystyle{\frac{
        \funfont{skipcat}: (\ImpConcat{\ImpSkip}{W}, s_0[x \leftarrow 1]) \rightsquigarrow^* (W, s_0[x \leftarrow 1])
        \qquad
        \cdots
      }{
        (\ImpConcat{\ImpSkip}{W}, s_0[x \leftarrow 1]) \rightsquigarrow^* (W, s_0[x \leftarrow 1][x \leftarrow 2])
      }}
  \]
  y por tanto:    
  \[
    \funfont{head}: \frac{
      \funfont{cat}: \displaystyle{\frac{
        \funfont{ass}: (\ImpAssign{x}{1}, s_0) \rightsquigarrow (\ImpSkip, s_0[x \leftarrow 1])
      }
      {(\ImpConcat{\ImpAssign{x}{1}}{W}, s_0)
      \rightsquigarrow
      (\ImpConcat{\ImpSkip}{W}, s_0[x \leftarrow 1])
      }}
      \qquad
      H
    }{
      (\ImpConcat{\ImpAssign{x}{1}}{W}, s_0) \rightsquigarrow^* (W, s_0[x \leftarrow 1][x \leftarrow 2])
    }
  \]
  que hemos obtenido aplicando sucesivamente los constructores. Notamos que
  dando \enquote{un paso más} llegaríamos al estado final del programa.
\end{example}

Presentamos ahora una serie de teoremas que serán muy útiles al probar la
equivalencia entre la semántica de paso largo y la de paso corto. Prestando atención
a los constructores de la relación de paso corto, podemos ver que el único
\enquote{especial} es \(\funfont{cat}\), ya que es el único que requiere un argumento.
Por esta razón, todos los teoremas siguientes nos darán resultados análogos para \(\rightsquigarrow^*\)
a los de la semántica de paso largo.

\begin{theorem}
  Si una instrucción evalúa a un estado, concatenarla con otra tiene al estado 
  generado como intermedio. Más precisamente, si \((c, s) \rightsquigarrow^* (\ImpSkip, s')\)
  entonces \((\ImpConcat{c}{c'}, s) \rightsquigarrow^* (\ImpConcat{\ImpSkip}{c'}, s')\).
\end{theorem}

\begin{proof}
  La demostración es consecuencia directa del teorema de elevación~\ref{thm:rel_lift}.
  Las relaciones \(\mathcal{R}\) y \(\mathcal{P}\) son ambas la semántica de paso corto
  \(\rightsquigarrow\), y la función \(f\) es la concatenación de instrucciones:
  \[f: \tyfont{Com} \times \tyfont{State} \to \tyfont{Com} \times \tyfont{State} \coloneqq \lambda (c, s) \mapsto (\ImpConcat{c}{c'}, s).\]
  Nuestra hipótesis \(h\) es la concatenación en la semántica de paso corto, es decir:
  \[h: \forall (c, s), (d, t), (c, s) \rightsquigarrow (d, t) \to (\ImpConcat{c}{c'}, s) \rightsquigarrow (\ImpConcat{d}{c'}, t)
     \coloneqq \lambda x \mapsto \funfont{cat} \ x\]
  y finalmente tenemos por hipótesis que \((c, s) \rightsquigarrow^* (\ImpSkip, s')\).
\end{proof}

\begin{corollary}
  Concatenar una instrucción que evalúa a una configuración intermedia con otra
  que evalúa en esa configuración nos da la configuración de la segunda.
  Es decir, si \((c, s) \rightsquigarrow^* (\ImpSkip, s')\)
  y \((c', s') \rightsquigarrow^* (\ImpSkip, s'')\) entonces
  \((\ImpConcat{c}{c'}, s) \rightsquigarrow^* (\ImpSkip, s'')\).
\end{corollary}

\begin{corollary}
  Concatenar una instrucción que no cambia el estado con otra es equivalente a 
  evaluar la segunda. Más precisamente, si \((c, s) \rightsquigarrow^* (\ImpSkip, s)\) entonces
  \((\ImpConcat{c}{c'}, s) \rightsquigarrow^* (c', s)\).
\end{corollary}

\section{Semántica denotacional relacional}

Hemos seguido aquí la construcción presentada en~\cite[Capítulo~11]{semantics_with_isabelle}
para definir una semántica denotacional \emph{relacional} para el lenguaje \ImpName.

Como vimos antes, definir una función que evalúe un programa no es trivial.
Para empezar, en general estas funciones podrán ser \emph{no totales}, ya que existen programas que no
terminan~\ref{thm:non_termination}. La \enquote{función} ha de ser entonces
\emph{parcial}, pudiendo no devolver un valor para algunos de sus argumentos.
Esto es un problema ya que en teoría de tipos todas las funciones son
totales. Para sortear este problema
podríamos usar la mónada \(\tyfont{Option}\) y devolver \(\funfont{none}\) cuando el
programa no termina:
\[
  \funfont{eval} : \tyfont{Com} \to \tyfont{State} \to \tyfont{Option} \ \tyfont{State}.
\]

Lean nos permite definir funciones parciales de esta forma, utilizando implícitamente
la teoría de las \(\omega\)-cadenas. Puesto que introducir mónadas requiere 
introducir toda una variedad de conceptos categóricos haremos una pequeña
introducción a la teoría subyacente a las \(\omega\)-cadenas usando
\emph{conjuntos}.

En lo que sigue usaremos la definición clásica de funciones (posiblemente parciales) 
como casos particulares de relaciones, es decir, 
grafos de \(\tyfont{Set} \ (\alpha \times \beta)\). Esto nos permitirá usar el
orden parcial de subconjuntos para definir la semántica del lenguaje
mediante el teorema de \emph{Knaster-Tarski}. 

También tendremos que asegurarnos de que las relaciones que definamos de esta forma
sean, en efecto, funciones parciales, es decir, que a todo valor de entrada le corresponde
a lo sumo un valor de salida. Pero ello queda garantizado por el hecho de que
dichas relaciones son límites de \(\omega\)-cadenas de funciones parciales,
las cuales son siempre, también, funciones parciales.

\subsection{Punto fijo de Knaster-Tarski}

El teorema de \emph{Knaster-Tarski} es una generalización del teorema de
\emph{Banach} del punto fijo, que establece que toda función monótona
en un retículo completo tiene un punto fijo mínimo. Este resultado será esencial 
para definir la semántica denotacional de los bucles. Pero antes de poder
enunciar el teorema tenemos que introducir bastantes definiciones.

\subsubsection{Teoría de orden}

Comenzamos definiendo un \emph{preorden}, que es la forma \enquote{más débil}
de orden que podemos definir.

\begin{definition}[Preorden]
  Un preorden es una relación binaria (usualmente denotada por \(\leq\)) que
  satisface las siguientes propiedades:
  \begin{itemize}
    \item Reflexividad: \(a \leq a\).
    \item Transitividad: \(a \leq b \to b \leq c \to a \leq c\).
  \end{itemize}
\end{definition}

Añadiendo la propiedad de \emph{antisimetría} al preorden obtenemos un \emph{orden parcial}.

\begin{definition}[Orden parcial]
  Un orden parcial es un preorden antisimétrico, es decir, satisface:
  \(a \leq b \to b \leq a \to a = b\).
\end{definition}

Consideramos ahora un conjunto de elementos con un orden parcial, si cada subconjunto
suyo tiene un \emph{ínfimo} lo llamamos un \emph{retículo completo}.
Normalmente un retículo completo se define como un conjunto parcialmente ordenado
en el que cada conjunto tiene un \emph{mínimo límite superior} y un \emph{máximo límite inferior},
esa definición y la que usamos son intercambiables, aunque no nos molestaremos en demostrarlo.

\begin{definition}
  Un retículo completo es un tipo parcialmente ordenado que tiene un ínfimo en cada conjunto.
  Es decir, existe una función \(\funfont{Inf}: \tyfont{Set} \ \alpha \to \alpha\)
  que devuelve un elemento del conjunto que satisface las siguientes propiedades:
  \begin{itemize}
    \item \(\forall S, \forall x \in S, \funfont{Inf} \ S \leq x\).
    \item \(\forall S, (\forall y \in S, x \leq y) \implies x \leq \funfont{Inf} \ S\).
  \end{itemize}
\end{definition}

Además este ínfimo es \emph{único}.

\begin{theorem}
  El ínfimo de un conjunto en un retículo completo es único.
  Es decir, si \(\funfont{Inf} \ s = a\) y \(\funfont{Inf} \ s = b\) entonces \(a = b\,\).
\end{theorem}

Ahora necesitamos recordar la definición de subconjuntos en~\ref{sec:sets},
y notar que los conjuntos, con la relación de inclusión, forman un orden parcial.

\begin{theorem}
  Los conjuntos con la relación de inclusión forman un orden parcial. Donde
  \(A \leq B \coloneqq A \subseteq B\), \(A < B \coloneqq A \subset B\)
  y el ínfimo de un conjunto es la intersección de todos sus subconjuntos,
  es decir \(\funfont{Inf} \ S = \bigcap_{A \in S} A\,\).
\end{theorem}

Donde la intersección de un conjunto de conjuntos es el conjunto de elementos que
están en todos los conjuntos del conjunto, es decir:
\[
  \bigcap_{A \in S} A = \{x \mid \forall A \in S, x \in A\}
\]
Las demostraciones se obtienen sin más que aplicar las definiciones.

\subsubsection{Homomorfismos de orden}\label{sec:order_homomorphisms}

Nos interesan particularmente las funciones que preservan el orden de los elementos
de un retículo, que se llaman \emph{homomorfismos de orden}, y como veremos
más adelante admiten siempre un \emph{mínimo punto fijo}.

\begin{definition}[Función monótona]
  Una función \(f: \alpha \to \alpha\) es monótona si para todos los \(a, b: \alpha\)
  se cumple que \(a \leq b \implies f \ a \leq f \ b\). Llamamos a estas funciones
  \emph{homomorfismos de orden}.
\end{definition}

\begin{notation}
  Denotaremos un homomorfismo de orden como \(f: \alpha \to_o \alpha \,\).
\end{notation}

Algunos ejemplos básicos de homomorfismos de orden son la función identidad y las funciones
constantes.

\begin{theorem}
  La función identidad es un homomorfismo de orden.
\end{theorem}

\begin{theorem}
  Cualquier función constante es un homomorfismo de orden.
\end{theorem}

\subsubsection{Puntos fijos}

Queremos demostrar ahora que los homomorfismos de orden tienen un punto fijo mínimo,
pero para ello tenemos que introducir aún más definiciones.

\begin{definition}[Pre-punto fijo]
  Un pre-punto fijo de una función \(f: \alpha \to_o \alpha\) es un punto \(a: \alpha\)
  tal que \(f \ a \leq a\,\).
\end{definition}

\begin{definition}[Punto fijo mínimo de Knaster-Tarski]
  El punto fijo mínimo (\emph{least fixed-point}) de una función \(f: \alpha \to_o \alpha\) es el ínfimo del
  conjunto de todos los pre-puntos fijos de \(f\):
  \[
    \funfont{lfp} \ f = \funfont{Inf} \ \{a \mid f \ a \leq a\}
  \]
\end{definition}

En el caso que nos ocupa, que es el de los conjuntos, esto se convierte en
\(\funfont{lfp} \ f = \bigcap \{a \mid f \ a \subseteq a\}\),
donde \(f: \tyfont{Set} \ \alpha \to \tyfont{Set} \ \alpha\,\).
Presentamos ahora dos teoremas sencillos pero importantes sobre los pre-puntos fijos.

\begin{theorem}
  El punto fijo mínimo de una función es menor o igual que cualquier
  pre-punto fijo de esa función. Es decir, si \(a\) es un pre-punto fijo
  de \(f\) entonces \(\funfont{lfp} \ f \leq a\,\).
\end{theorem}

\begin{corollary}
  Si \(x\) es un punto fijo de \(f\), es decir, \(f \ x = x\), entonces
  \(\funfont{lfp} \ f \leq x\).
\end{corollary}

\begin{theorem}\label{thm:lfp_le}
  Si un punto es menor o igual que cualquier pre-punto fijo de una función
  entonces es menor o igual que el punto fijo mínimo de esa función.
  Es decir, si para todos los pre-puntos fijos \(b\) de \(f\) tenemos
  \(a \leq b\) entonces \(a \leq \funfont{lfp} \ f\,\).
\end{theorem}

Podemos ahora enunciar, por fin, el teorema de \emph{Knaster-Tarski}, que establece que
el punto fijo mínimo de una función monótona en un retículo completo 
\emph{es un punto fijo}.

\begin{theorem}[Knaster-Tarski]\label{thm:lfp_eq}
  Sea \(f: \alpha \to_o \alpha\) una función monótona en un retículo completo,
  entonces \(f\) tiene un punto fijo, y es igual al punto fijo mínimo
  de \(f\). En otras palabras:
  \[
    \funfont{lfp} \ f = f \ (\funfont{lfp} \ f)
  \]
\end{theorem}

\subsection{Relaciones conjuntistas}

Todo este desarrollo que hemos hecho sería inútil si tras el no pudiéramos definir las 
funciones (relaciones) que nos interesan. Para ello usaremos la
\emph{teoría de conjuntos} y definiremos las relaciones como conjuntos de pares.
Podemos \enquote{definir}, por ejemplo, la \enquote{función identidad} como el grafo
de la identidad.

\begin{definition}
  El grafo de la función identidad \(\funfont{SRel.id}: \tyfont{Set} (\alpha \times \alpha)\) es el
  conjunto de pares: \(\funfont{SRel.id} = \{(x, y) \mid x = y\}\,\).
\end{definition}

Podemos también definir la composición de relaciones utilizando los elementos
comunes entre el codominio de la primera relación y el dominio de la segunda.

\begin{definition}
  La composición de dos relaciones \(f: \tyfont{Set} (\alpha \times \beta)\) y \(g: \tyfont{Set} (\beta \times \gamma)\)
  es el conjunto de pares:
  \[
    f \circ g \, \coloneqq \, \{(x, z) \mid \exists y, (x, y) \in f \land (y, z) \in g\}
  \]
\end{definition}

Para ver que lo que estamos definiendo tiene sentido podemos comprobar que
la composición de una relación con la identidad es la relación misma.

\begin{theorem}
  La composición de una relación con la identidad por la derecha es la relación misma.
  Es decir \(f \circ \funfont{SRel.id} = f\,\).
\end{theorem}

Se da lo mismo por la izquierda. Más importante aún, podemos probar que la composición
de relaciones es monótona.

\begin{theorem}\label{thm:rel_comp_monotone}
  La composición de dos \enquote{funciones} es monótona, es decir, si \(f \subseteq h\) y
  \(g \subseteq k\), entonces \(f \circ g \subseteq h \circ k\,\).
\end{theorem}

La última pieza antes de definir la semántica denotacional es la definición
del \emph{conjunto if-else}.

\begin{definition}[Conjunto if-else]
  El \emph{conjunto if-else} (\(\funfont{Set.ite}\)) es una función que dados tres conjuntos \(P, A, B\)
  devuelve el conjunto:
  \[
    \funfont{Set.ite} \ P \ A \ B = A \cap P \cup B \setminus P
  \]
  donde \(P\) es el \enquote{predicado} o \emph{conjunto de condiciones}.
\end{definition}

\begin{theorem}\label{thm:ite_monotone}
  La función \(\funfont{Set.ite}\) es monótona en sus argumentos.
  Es decir, si \(a \subseteq a'\) y \(b \subseteq b'\) entonces
  \[
  \funfont{Set.ite} \ p \ a \ b \subseteq \funfont{Set.ite} \ p \ a' \ b'
  \]
\end{theorem}

\subsection{Semántica denotacional}

Podemos por fin definir nuestra función que dada una instrucción devuelve la
\enquote{función} de estado a estado que representa la evaluación de esa instrucción.

\begin{definition}[Denotación]
Definimos la función \(\funfont{denote}: \tyfont{Com} \to \tyfont{Set} (\tyfont{State} \times \tyfont{State})\)
como:
  \begin{align*}
    \funfont{denote} \ \ImpSkip &\coloneqq \funfont{SRel.id} \\
    \funfont{denote} \ \ImpAssign{v}{a} &\coloneqq \{(s, t) \mid t = s[v \leftarrow a \ s]\} \\
    \funfont{denote} \ \ImpConcat{c}{c'} &\coloneqq \funfont{denote} \ c \circ \funfont{denote} \ c' \\
    \funfont{denote} \ (\ImpIfElse{b}{c}{c'}) &\coloneqq \funfont{Set.ite} \ \{(s, \placeholder) \mid b \ s\} \ (\funfont{denote} \ c) \ (\funfont{denote} \ c') \\
    \funfont{denote} \ (\ImpWhile{b}{c}) &\coloneqq \funfont{lfp} \ (W \ b \ (\funfont{denote} \ c))
  \end{align*}
  donde \(W \ b \ f:\tyfont{Set} \ (\tyfont{State} \times \tyfont{State}) \to_o 
    \tyfont{Set} \ (\tyfont{State} \times \tyfont{State})\) es:
  \[
    W \ b \ f = \lambda g \mapsto \funfont{Set.ite} \ \{(s, \placeholder) \mid b \ s\} \ (f \circ g) \ \funfont{id}
  \]
\end{definition}

Por supuesto, esto solo tiene sentido si el punto fijo mínimo de \(W \ b \ f\) existe.
Para ver esto basta demostrar que \(W \ b \ f\) es un homomorfismo de orden, 
ya que entonces podemos aplicar el teorema de \emph{Knaster-Tarski}. 
Esto se demuestra fácilmente como corolario de los teoremas~\ref{thm:rel_comp_monotone} y~\ref{thm:ite_monotone}.

\begin{theorem}
  La función \(W \ b \ f\) es un homomorfismo de orden.
\end{theorem}

En realidad el punto fijo de \(W \ b \ f\) es una \emph{relación} entre el primer
estado en el que se evalúa el \enquote{while} y el último estado, en caso de
que el bucle termine. En particular, si el bucle no termina nunca, la relación es el \emph{conjunto vacío},
es decir, no hay estado relacionado con ningún otro estado.
Vemos un ejemplo concreto de lo que pasa con un bucle infinito.

\begin{example}
Sea \(L \coloneqq \ImpWhile{\ImpTrue}{\ImpSkip}\),
aplicando la definición de la semántica denotacional tenemos:
\[
  \funfont{denote} \ L = \funfont{lfp} \ (W \ \ImpTrue \ \funfont{SRel.id})
\]
ahora desplegamos la definición de \(W\):
\begin{align*}
  W \ \ImpTrue \ \funfont{SRel.id} &= \lambda g \mapsto \funfont{SRel.id} \circ g \cap \funfont{Univ} \cup \funfont{SRel.id} \setminus \funfont{Univ} \\
    &= \lambda g \mapsto g = \funfont{id}
\end{align*}
Y el punto fijo mínimo es:
\[
  \funfont{lfp} \ \funfont{id} = \bigcap \{a \mid \funfont{id} \ a \leq a\} = 
    \bigcap \funfont{Univ} = \emptyset
\]
Con lo que hemos demostrado que el bucle no termina nunca.
\end{example}

\begin{notation}
  Denotamos la semántica denotacional de una instrucción \(c\) como
  \(\Denote{c}\,\). Si al evaluar \(c\) en un estado \(s\)
  obtenemos un estado \(s'\) escribimos:
  \[
    (s, s') \in \Denote{c}
  \]
\end{notation}

Con lo que antes hemos demostrado que \(\forall s \ s', (s, s') \notin \Denote{\ImpWhile{\ImpTrue}{\ImpSkip}}\).
La equivalencia de comportamiento en esta semántica es fácil de definir.

\begin{definition}
  Dos instrucciones \(c\) y \(c'\) son equivalentes en la semántica denotacional
  si \(\Denote{c} = \Denote{c'}\,\).
  Entonces escribiremos \(c \sim c'\).
\end{definition}

Con esto podemos obtener de forma inmediata resultados como el siguiente.

\begin{theorem}\label{thm:while_unroll_denote}
  Podemos desenrollar todo bucle \texttt{while}. Esto es:
  \[\Denote{\ImpWhile{b}{c}} = \Denote{\ImpIfElse{b}{\ImpConcat{c}{\ImpWhile{b}{c}}}{\ImpSkip}}.\]
\end{theorem}

\subsection{Punto fijo de Kleene}

Esta definición de la semántica denotacional funciona, y como hemos visto
nos permite hacer algunas demostraciones interesantes, como ver que un programa
no termina\footnote{Supuesto que la semántica denotacional es equivalente a la
  de paso largo, lo cual veremos en~\ref{sec:equivalence}.}.
Sin embargo, la definición de la semántica denotacional es 
\enquote{computacionalmente incómoda}, ya que no podemos ir \enquote{dando pasos}
para calcular el punto fijo mínimo de una función. Para ello necesitamos
introducir una teoría, originalmente desarrollada por \emph{Dana Scott}, que
nos permite usar \emph{funciones parciales} en lugar de relaciones.
Demostraremos entonces que el teorema de \emph{Kleene} nos permite
aproximarnos al punto fijo mínimo de una función \emph{\(\omega\)-continua}.

Comenzamos definiendo el concepto de \(\omega\)-cadena, que es una secuencia de
conjuntos donde cada conjunto es un subconjunto del siguiente.

\begin{definition}[\(\omega\)-cadena]
  Una \(\omega\)-cadena es una secuencia de conjuntos \({\{S_i\}}_{i \in \mathbb{N}}\) tal que
  \(S_i \subseteq S_{i+1}\) para todo \(i \in \mathbb{N}\,\).
\end{definition}

Una función \(\omega\)-continua es una función que \enquote{preserva el límite} de
\(\omega\)-cadenas.

\begin{definition}[\(\omega\)-continuidad]
  Una función \(f: \alpha \to \alpha\) es \(\omega\)-continua si conmuta con la unión
  indexada para cada cadena, es decir, para cada \(\omega\)-cadena \({\{S_i\}}_{i \in \mathbb{N}}\), se cumple:
  \[
    f \left( \bigcup_{i \in \mathbb{N}} S_i \right) = \bigcup_{i \in \mathbb{N}} f \ S_i
  \]
\end{definition}

\begin{notation}
  Denotamos que una función es \(\omega\)-continua por \(f: \alpha \to_\omega \alpha\).
\end{notation}

La continuidad es una propiedad \enquote{más fuerte} que la monotonía, ya que
cada función continua es monótona, pero no toda función monótona es continua.

\begin{theorem}
  Toda función \(\omega\)-continua es monótona.
\end{theorem}

Consideramos como ejemplo de \(\omega\)-cadena la secuencia de \emph{aplicaciones reiteradas},
denotadas como las potencias \(f^n\),
de un homomorfismo de orden de conjuntos a conjuntos.

\begin{theorem}\label{thm:fpow_chain}
  La secuencia de aplicaciones reiteradas de un homomorfismo de orden \(f: \tyfont{Set} \ \alpha \to_o \tyfont{Set} \ \alpha\)
  es una \(\omega\)-cadena. Es decir, \({\{f^n \ \emptyset\}}_{n \in \mathbb{N}}\) es una \(\omega\)-cadena.
\end{theorem}

Esto será útil más adelante, ya que demostraremos que el punto fijo mínimo de una función
\(\omega\)-continua es un \emph{punto fijo}.

\begin{definition}[Punto fijo mínimo de Kleene]
  El punto fijo mínimo de una función \(f: \alpha \to_\omega \alpha\) es el
  \enquote{límite} de la secuencia de las aplicaciones reiteradas de \(f\), es decir:
  \[
    \funfont{lfp} \ f = \bigcup_{n \in \mathbb{N}} f^n \ \emptyset
  \]
\end{definition}

Podemos ahora enunciar el teorema de \emph{Kleene}.

\begin{theorem}[Kleene]
  Si \(f: \alpha \to_\omega \alpha\) es una función \(\omega\)-continua
  entonces el punto fijo mínimo de \(f\) existe, y es igual al punto fijo mínimo
  de Knaster-Tarski.
\end{theorem}

\begin{proof}
  Queremos demostrar que \(\funfont{OrderHom.lfp} \ f = \funfont{ContinuousHom.lfp} \ f\),
  para ello demostramos que los conjuntos se contienen mutuamente.
  \begin{itemize}
    \item[(\(\subseteq\))] 
      Basta demostrar que \(\bigcup_{n \in \mathbb{N}} f^n \ \emptyset\)
      es un pre-punto fijo de \(f\), ya que entonces podemos aplicar~\ref{thm:lfp_le},
      obteniendo el resultado. Que el punto fijo mínimo es un pre-punto fijo es más o menos
      inmediato, usando~\ref{thm:fpow_chain}.

    \item[(\(\supseteq\))] 
      En este caso basta demostrar que \(\funfont{lfp} \ f \supseteq f^n \ \emptyset\).
      Procedemos por inducción sobre \(n\). El caso base es trivialmente contradictorio,
      el paso inductivo es inmediato aplicando Knaster-Tarski~\ref{thm:lfp_eq},
      ya que \(f\) es un homomorfismo de orden.
  \end{itemize}
\end{proof}

Esta definición de punto fijo tiene la ventaja de darnos una aproximación
computacional del punto fijo mínimo, ya que podemos tomar la unión de las primeras \(n\)
potencias de \(f\), y si en alguna iteración la unión no cambia, podemos
detenernos y devolver el resultado. 

Como anticipamos al principio de la sección, Lean usa internamente esta 
teoría para permitirnos definir funciones parciales, cuyo codominio es 
\(\tyfont{Option} \ A\), mediante la táctica \texttt{partial\_fixpoint},
véase la función \texttt{eval} en~\ref{sec:code_denotational}, donde
hacemos uso de esto para dar una definición más \enquote{inmediata} de la función de evaluación.
Igualmente nuestra definición basada en conjuntos nos permite demostrar más fácilmente algunos
resultados, como el de no terminación.

\section{Equivalencia de semánticas}\label{sec:equivalence}

Llegamos ya al resultado más importante del trabajo, que muestra que las semánticas que hemos
definido son todas ellas equivalentes, por lo que dependiendo de lo que estemos estudiando
elegiremos la que más nos convenga. La demostración de esto es larga y
tortuosa, por lo que la dividimos en varios teoremas. Comenzamos demostrando
que la semántica estructural es equivalente a la semántica de paso largo.

\begin{theorem}
  La semántica natural \enquote{implica} la estructural. Es decir, si
  \(x\) es una configuración inicial y \(s'\) un estado tal que
  \(x \bigstep s'\) entonces \(x \rightsquigarrow^* (\ImpSkip, s')\).
\end{theorem}

Para la implicación inversa introducimos primero un lema.

\begin{lemma}
  Sean \(x\) y \(x'\) configuraciones y \(s'\) un estado tal que
  \(x \rightsquigarrow x'\) y \(x' \bigstep s'\), entonces
  \(x \bigstep s'\).
\end{lemma}

Con esto obtenemos el resultado sin más que aplicar inducción en la cabeza de la
relación reflexiva transitiva.

\begin{theorem}
  La semántica estructural implica la semántica natural. Es decir, si
  \(x\) es una configuración inicial y \(s'\) un estado tal que
  \(x \rightsquigarrow^* (\ImpSkip, s')\) entonces \(x \bigstep s'\).
\end{theorem}

Ahora demostraremos que la semántica de paso largo es equivalente a la
denotacional y con ello habremos terminado. Empezamos por la dirección fácil,
que es natural implica denotacional. El resultado se obtiene mediante inducción
en los constructores del paso largo y desenrollando el \texttt{while} en la
semántica denotacional~\ref{thm:while_unroll_denote}.

\begin{theorem}
  La semántica natural \enquote{implica} la semántica denotacional. Es decir, sean
  \(s \ s': \funfont{State}\) y \(c: \tyfont{Com}\) tales que
  \((c, s) \bigstep s'\), entonces \((s, s') \in \Denote{c}\,\).
\end{theorem}

Tenemos también la \enquote{implicación contraria}.

\begin{theorem}
  La semántica denotacional implica la semántica natural. Es decir, sean
  \(s \ s': \funfont{State}\) y \(c: \tyfont{Com}\) tales que
  \((s, s') \in \Denote{c}\), entonces \((c, s) \bigstep s'\).
\end{theorem}

En este caso hacemos la inducción sobre la forma de \(c\,\). El único caso
problemático es el del \texttt{while}, que se resuelve aplicando el
siguiente lema.

\begin{lemma}
  Se cumple que \(\Denote{\ImpWhile{b}{c}} \subseteq \{(s, s'') \mid (\ImpWhile{b}{c}, s) \bigstep s'' \}\,\).
\end{lemma}

Esto se cumple gracias a la monotonía de la semántica denotacional.

Como las dos semánticas son equivalentes a la natural, también son
equivalentes entre sí. En resumen, tenemos que:
\begin{theorem}[Equivalencia de semánticas]
  La semántica de paso largo, la semántica estructural y la semántica denotacional
  son equivalentes. Es decir, se cumple que:
  \[
    (c, s) \bigstep s'\, \longleftrightarrow 
      \, (c, s) \rightsquigarrow^* (\ImpSkip, s')\, \longleftrightarrow \, (s, s') \in \Denote{c}
  \]
\end{theorem}

\chapter*{Conclusiones}

\addcontentsline{toc}{chapter}{Conclusiones}  

Durante el desarrollo de este trabajo hemos visto como las herramientas de 
la teoría de tipos y los demostradores automáticos nos permiten formalizar
resultados vistos a lo largo de la carrera, más allá de cualquier atisbo de duda.
Los resultados vistos son una parte de lo que se cubre en
el curso de \emph{Teoría de la programación}, pero las demostraciones son
completamente distintas. En efecto adaptar esas demostraciones a un
marco formal como el de Lean es un trabajo arduo y tedioso, que además
requiere conocimientos sobre programación funcional, que no se 
cubren a lo largo de la carrera.

La ventaja didáctica del esfuerzo realizado en este texto es clara,
allá donde el corpus visto en clase contenía \enquote{agujeros} hemos
tenido que detenernos a definir cuidadosamente todos los puntos
que normalmente se dan por supuestos. Además, esto nos ha permitido
convencernos de que esta teoría es válida a nivel formal, y no
solo a nivel \enquote{intuitivo}.

El resultado más importante que hemos obtenido es la equivalencia
entre las semánticas de paso largo, estructural y denotacional, cuya demostración, como
hemos visto, no es especialmente larga una vez hemos introducido la
maquinaria necesaria.

Con este pequeño ejemplo de formalización hemos visto por qué la teoría de
tipos y los asistentes de demostración están ganando tanta popularidad
y sus ventajas de cara a asegurar la validez de los resultados obtenidos.

% Bibliography

\bibliographystyle{alpha}
\bibliography{refs}

% Appendices 

\appendix

\addcontentsline{toc}{part}{Apéndices}

\chapter{Listado de código}

El código del texto se encuentra en el repositorio de GitHub
\url{https://github.com/remind-me-later/SemanticsLean}. 
Lo reproducimos aquí por completitud, con una sección para cada
archivo.

\section{Mapas}

\inputminted{lean4}{../SemanticsLean/Semantics/Maps.lean}

\section{Conjuntos}

\inputminted{lean4}{../SemanticsLean/Semantics/Set.lean}

\section{Sintaxis de IMP}

\inputminted{lean4}{../SemanticsLean/Semantics/Imp/Lang.lean}

\section{Expresiones aritméticas}

\inputminted{lean4}{../SemanticsLean/Semantics/Imp/Aexp.lean}

\section{Expresiones booleanas}

\inputminted{lean4}{../SemanticsLean/Semantics/Imp/Bexp.lean}

\section{Semántica de paso largo}

\inputminted{lean4}{../SemanticsLean/Semantics/Imp/BigStep.lean}

\section{Clausura reflexiva transitiva}\label{sec:code_refltransgen}

\inputminted{lean4}{../SemanticsLean/Semantics/ReflTransRel.lean}

\section{Semántica estructural}

\inputminted{lean4}{../SemanticsLean/Semantics/Imp/SmallStep.lean}

\section{Relaciones conjuntistas}
\inputminted{lean4}{../SemanticsLean/Semantics/SetRelation.lean}

\section{Orden y punto fijo de Knaster-Tarski}

\inputminted{lean4}{../SemanticsLean/Semantics/Lattice.lean}

\section{Cadenas y punto fijo de Kleene}

\inputminted{lean4}{../SemanticsLean/Semantics/Chain.lean}

\section{Semántica denotacional}\label{sec:code_denotational}

\inputminted{lean4}{../SemanticsLean/Semantics/Imp/Denotational.lean}

\section{Equivalencia de semánticas}

\inputminted{lean4}{../SemanticsLean/Semantics/Imp/Equivalence.lean}


\end{document}
