...
\begin{definition}[Conjunto]
  ...
  proposición.
  \[
  \tyfont{Set} \ (\alpha: \texttt{Type}) \coloneqq \alpha \to \Prop 
  \]
\end{definition}
...
\begin{proof}
  ...
  \(\texttt{refl}: a \mathcal{R}^* a\) implica que \(a = b\), así que
  \((f \ a) \mathcal{P}^* (f \ a)\)
...
\enquote{especial} es \texttt{cat}, ya que es el único que requiere un argumento.
...
\begin{proof}
  ...
  \[f: \texttt{Com} \times \texttt{State} \to \texttt{Com} \times \texttt{State} \coloneqq \lambda (c, s) \mapsto (\ImpConcat{c}{c'}, s).\]
...
y definimos una semántica denotacional \emph{relacional} para el lenguaje \texttt{IMP}.
...
podríamos usar la mónada \texttt{Option} y devolver \texttt{none} cuando el
programa no termina:
\[
...
son funciones parciales, para evitar manchar la semántica de \texttt{IMP} con
...
\begin{definition}[Punto fijo mínimo]
...
del \emph{conjunto if-then-else}

\begin{definition}[Conjunto if-then-else]
  El \emph{conjunto if-then-else} (\(\funfont{Set.ite}\)) es una función que dados tres conjuntos \(P, A, B\)
...
para ver esto basta demostrar que \(W \ b \ f\) es un homomorfismo de orden, ya que
entonces podemos aplicar el teorema de \emph{Knaster-Tarski}.
...
\subsection{Clausura reflexiva transitiva}\label{sec:code_refltransgen}
