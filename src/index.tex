...
\begin{itemize}
\item Parts I'm not sure about or need sources are \hl{marked like this}.
\end{itemize}
...
\hl{It emerged in the early 20th century to solve the foundational crisis of
...
Modern type theory is arises from the work of Pier Martin-Löf, who developed an
\emph{intuitionistic} type theory in the 1970s. In intuitionistic type theory
...
some caveats). Another important feature of Martin-Löf type theory is that it
has a \emph{computational interpretation} via the Curry-Howard isomorphism, which
we will discuss in more detail later.
This computational interpretation is what 
allows computers to reason about mathematical proofs.

To achieve this computational interpretation, Martin-Löf type theory combines
classic type theory with the \(\lambda\)-calculus.
...
abstraction (\(\lambda x \implies M\)), and application (\(M \ M\)), in 
BNF:
\begin{align*}
  M & \Coloneqq x \mid \lambda x \implies M \mid M \ M
\end{align*}
...
\begin{itemize}
  \item \(\alpha\)-conversion: Allows us to rename bound variables:
    \[(\lambda x \implies M) \ N \longrightarrow M[x \coloneqq N]\]
  ...
  \item \(\eta\)-conversion: Expresses the concept of \emph{extensionality} of functions, 
    ...
    \[(\lambda x \implies f \ x) \longrightarrow f\]
\end{itemize}
...
\begin{definition}[\(\Omega\)-combinator]\label{def:omega_combinator}
  ...
  \[
  \Omega \coloneqq (\lambda x \implies x \ x) \ (\lambda x \implies x \ x)
  \]
...
\begin{definition}[Church booleans]
  ...
  \begin{align*}
    \texttt{true} & \coloneqq \lambda x \implies \lambda y \implies x \\
    \texttt{false} & \coloneqq \lambda x \implies \lambda y \implies y
  \end{align*}
\end{definition}
...
\begin{definition}[Logical operators]
  ...
  \begin{align*}
    \texttt{and} & \coloneqq \lambda x \implies \lambda y \implies x \ y \ x \\
    \texttt{or} & \coloneqq \lambda x \implies \lambda y \implies x \ x \ y \\
    \texttt{not} & \coloneqq \lambda x \implies x \ \texttt{false} \ \texttt{true}
  \end{align*}
\end{definition}
...
\begin{definition}[If-then-else]
  ...
  \[
  \texttt{ifthen} \coloneqq \lambda b \implies \lambda x \implies \lambda y \implies b \ x \ y
  \]
\end{definition}
...
that can be used to reason about computation, \hl{it is in fact \emph{Turing complete},
...
function abstraction now has the form \(\lambda x: \alpha \implies M\), where
...
\begin{align*}
  M \Coloneqq & \ x \mid \lambda x: \alpha \implies M \mid M \ M \\
...
introduce the concepts of the Calculus of Constructions.
...
\begin{definition}[Type theoretic set]
  ...
  \[
  \texttt{Set} \ (\alpha : \texttt{Type}) \coloneqq \alpha \to \texttt{Prop}
  \]
\end{definition}
...
\begin{definition}[Relation]
  ...
  \texttt{Rel} \ (\alpha : \texttt{Type}) \ (\beta : \texttt{Type}) 
    \coloneqq \alpha \to \beta \to \texttt{Prop}
  \]
\end{definition}
...
\[
c \Coloneqq \texttt{skip} \ | \ x := a \ | \ c_1; c_2 \ | \
...
\begin{align*}
  \texttt{val}_1 &\coloneqq \mathbb{Z} \to \texttt{Aexp} \\
  \texttt{var}_1 &\coloneqq \texttt{String} \to \texttt{Aexp} \\
  \texttt{add}_1 &\coloneqq \texttt{Aexp} \to \texttt{Aexp} \to \texttt{Aexp} \\
  \texttt{sub}_1 &\coloneqq \texttt{Aexp} \to \texttt{Aexp} \to \texttt{Aexp} \\
  \texttt{mul}_1 &\coloneqq \texttt{Aexp} \to \texttt{Aexp} \to \texttt{Aexp}
\end{align*}
...
fashion:
\begin{align*}
  \texttt{true}_1 &\coloneqq \texttt{Bexp} \\
  \texttt{false}_1 &\coloneqq \texttt{Bexp} \\
  \texttt{not}_1 &\coloneqq \texttt{Bexp} \to \texttt{Bexp} \\
  \texttt{and}_1 &\coloneqq \texttt{Bexp} \to \texttt{Bexp} \to \texttt{Bexp} \\
  \texttt{or}_1 &\coloneqq \texttt{Bexp} \to \texttt{Bexp} \to \texttt{Bexp} \\
  \texttt{eq}_1 &\coloneqq \texttt{Aexp} \to \texttt{Aexp} \to \texttt{Bexp} \\
  \texttt{le}_1 &\coloneqq \texttt{Aexp} \to \texttt{Aexp} \to \texttt{Bexp}
\end{align*}
And the same goes for commands:
\begin{align*}
  \texttt{skip}_1 &\coloneqq \texttt{Com} \\
  \texttt{assign}_1 &\coloneqq \texttt{String} \to \texttt{Aexp} \to \texttt{Com} \\
  \texttt{seq}_1 &\coloneqq \texttt{Com} \to \texttt{Com} \to \texttt{Com} \\
  \texttt{if}_1 &\coloneqq \texttt{Bexp} \to \texttt{Com} \to \texttt{Com} \to \texttt{Com} \\
  \texttt{while}_1 &\coloneqq \texttt{Bexp} \to \texttt{Com} \to \texttt{Com}
\end{align*}
...
\begin{definition}[Initial state]
  ...
  \[
  s_0 \coloneqq \lambda x \implies 0
  \]
\end{definition}
...
\begin{align*}
  \funfont{reduce} \ (\texttt{val}_1 \ v) \ \cdot &\coloneqq v \\
  \funfont{reduce} \ (\texttt{var}_1 \ x) \ s &\coloneqq s \ x \\
  \funfont{reduce} \ (\texttt{add}_1 \ a_1 \ a_2) \ s &\coloneqq \funfont{reduce} \ a_1 \ s + \funfont{reduce} \ a_2 \ s \\
  \funfont{reduce} \ (\texttt{sub}_1 \ a_1 \ a_2) \ s &\coloneqq \funfont{reduce} \ a_1 \ s - \funfont{reduce} \ a_2 \ s \\
  \funfont{reduce} \ (\texttt{mul}_1 \ a_1 \ a_2) \ s &\coloneqq \funfont{reduce} \ a_1 \ s * \funfont{reduce} \ a_2 \ s
\end{align*}
...
\begin{align*}
  \texttt{true} &: (\texttt{true}, \cdot) \bigstep \texttt{true} \\
  \texttt{false} &: (\texttt{false}, \cdot) \bigstep \texttt{false} \\
  \texttt{not} &: \frac{(b, s) \bigstep b'}{\lnot (b, s) \bigstep \lnot b'} \\
  ...
  \texttt{eq} &: (a_1 = a_2, s) \bigstep a_1 \ s = a_2 \ s \\
...
\begin{align*}
  \funfont{reduce} \ \texttt{skip}_1 \ s &\coloneqq s \\
  \funfont{reduce} \ (x := a) \ s &\coloneqq s[x \leftarrow a \ s] \\
  \funfont{reduce} \ (c_1; c_2) \ s &\coloneqq \funfont{reduce} \ c_2 \ (\funfont{reduce} \ c_1 \ s) \\
  \funfont{reduce} \ (\texttt{if} \ b \ \texttt{then} \ c_1 \ \texttt{else} \ c_2 \ \texttt{end}) \ s &\coloneqq
    \text{if} \ b \ s \ \text{then} \ \funfont{reduce} \ c_1 \ s \ \text{else} \ \funfont{reduce} \ c_2 \ s \\
  \funfont{reduce} \ (\texttt{while} \ b \ \texttt{loop} \ c \ \texttt{end}) \ s &\coloneqq
...
\end{verbatim}
  ...
  \[
  \texttt{cat}: \frac{
    \texttt{ass}: (x := 2, s_0) \bigstep s_0[x \leftarrow 2]
    \quad
    \texttt{ifFalse}: \displaystyle{\frac{
      \texttt{ass}: (z := 4, s_0[x \leftarrow 2]) \bigstep s_0[x \leftarrow 2][z \leftarrow 4]
    }
    {
