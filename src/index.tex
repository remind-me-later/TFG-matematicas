...
\begin{itemize}
  \item \(\alpha\)-conversion: Allows us to rename bound variables:
    \[(\lambda x \mapsto M) \ N \longrightarrow M[x \coloneqq N]\]
  ...
  \item \(\eta\)-conversion: Expresses the concept of \emph{extensionality} of functions, 
    ...
    \[(\lambda x \mapsto f \ x) \longrightarrow f\]
\end{itemize}
...
\begin{example}
  ...
  \[
  (\lambda x \mapsto x + 1) \ 3 \longrightarrow 3 + 1 
  \]
\end{example}
...
\begin{proof}
  ...
  \[
  (\lambda x \mapsto x \ x) \ (\lambda x \mapsto x \ x) \longrightarrow
...
\begin{definition}[Subset]
  ...
  \[
  A \subset B \coloneqq \forall x, x \in A \mapsto x \in B 
  \]
\end{definition}
...
\begin{align*}
  \funfont{reduce} \ (\texttt{val} \ v) \ \cdot &\coloneqq v \\
  \funfont{reduce} \ (\texttt{var} \ x) \ s &\coloneqq s \ x \\
  \funfont{reduce} \ (\texttt{add} \ a_1 \ a_2) \ s &\coloneqq \funfont{reduce} \ a_1 \ s + \funfont{reduce} \ a_2 \ s \\
  \funfont{reduce} \ (\texttt{sub} \ a_1 \ a_2) \ s &\coloneqq \funfont{reduce} \ a_1 \ s - \funfont{reduce} \ a_2 \ s \\
  \funfont{reduce} \ (\texttt{mul} \ a_1 \ a_2) \ s &\coloneqq \funfont{reduce} \ a_1 \ s * \funfont{reduce} \ a_2 \ s
\end{align*}
...
\begin{align*}
  \funfont{reduce} \ \texttt{skip} \ s &\coloneqq s \\
...
\to \tyfont{Com} \times \tyfont{State} \to \tyfont{Prop}\) as:
  \[
...
\begin{example}
  ...
  {(\texttt{x := 1; while x <= 2 loop x := x + 1 end}, s_0)
  \rightsquigarrow
  (\texttt{skip; while x <= 2 loop x := x + 1 end}, s_0[x \leftarrow 1])
  }
  \]
\end{example}
