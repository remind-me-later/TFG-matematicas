...
\[
  \texttt{val} : (n, \square) \bigstep n
\]

Which relates the configuration \((n, \square)\), where the \(n\) of the left is
...
we will omit it for brevity) and the \(\square\) denotes any state, to the
...
\begin{align*}
  \funfont{eval} \ (\texttt{val} \ v) \ \square &\coloneqq v \\
...
\[
\begin{gathered}
  \texttt{true}: (\texttt{true}, \square) \bigstep \texttt{true} \quad
  \texttt{false}: (\texttt{false}, \square) \bigstep \texttt{false} \\
...
that is when we have a configuration of the form \((\texttt{skip}, \square)\).
...
\(\funfont{Inf} \ S = \bigcap_{A \in S} A\), we will not define the general 
indexed intersection since we will only use it here. Since the proofs are
straightforward we will not present them, but they can be found in the code.

Now we can begin defining the concepts of \emph{pre fix-point} and \emph{monotonic}
functions.

\begin{definition}[Pre fix-point]
  A pre fix-point of a function \(f: \alpha \to \alpha\) is a point \(a: \alpha\) 
  such that \(f \ a \leq a\).
\end{definition}


\begin{definition}[Least pre fix-point]
  The least pre fix-point of a function \(f: \alpha \to \alpha\) is the infimum of 
  the set of all pre fix-points of \(f\).
  \[
    \funfont{lfp} \ f = \funfont{Inf} \ \{a \mid f \ a \leq a\}
  \]
\end{definition}

As before, we can read it as \(\funfont{lfp} \ f = \bigcap \{a \mid f \ a \leq a\}\),
which in the case of sets is \(\funfont{lfp} \ f = \bigcap \{a \mid f \ a \subseteq a\}\),
that is the intersection of all the pre fix-points of \(f\).

Now we present two important pre fix-point inequalities.

\begin{theorem}
  The least pre fix-point of a function is less than or equal to any
  pre fix-point of that function. That is if \(a\) is a pre fix-point
  of \(f\) then \(\funfont{lfp} \ f \leq a\).
\end{theorem}

\begin{theorem}
  If a point is less than or equal to any pre fix-point of a function
  then it is less than or equal to the least pre fix-point of that function.
  If for all pre fix-points \(b\) of \(f\) we have \(a \leq b\) then
  \(a \leq \funfont{lfp} \ f\).
\end{theorem}

The last step before being able to enunciate the \emph{Knaster-Tarski} theorem
is defining the concept of a \emph{monotonic} function.

\begin{definition}[Monotonic function]
  A function \(f: \alpha \to \alpha\) is monotonic if for all \(a, b: \alpha\)
  we have that \(a \leq b \implies f \ a \leq f \ b\).
\end{definition}

That is, a monotonic function preserves the order of the elements of the set.
Some functions that conserve the order are the \emph{identity} function and the
constant functions. 

\begin{theorem}
  The identity function is monotonic.
\end{theorem}

\begin{theorem}
  A constant function is monotonic.
\end{theorem}

Now we can state the \emph{Knaster-Tarski} fix-point theorem.

\begin{theorem}[Knaster-Tarski]
  If \(f: \alpha \to \alpha\) is a monotonic function on a complete lattice
  then the least fix-point of \(f\) exists and is equal to the least
  pre fix-point of \(f\). In other words:
  \[
...
As we said before we will use the set theoretic view of functions as relations
to define the denotational semantics of the language. 

\begin{notation}
  We will denote the set of pairs of elements of \(\alpha\) and \(\beta\) as
  \(\alpha \to_s \beta \coloneqq \tyfont{Set} \ (\alpha \times \beta)\).
\end{notation}

This notation captures the idea of a function as a set of pairs, in reality
it is a \emph{partial multifunction} as it can have more than one pair with
the same first element. This will not be an issue for us, as the 
only function that we will define is single-valued.

With our brand new notation in place we can define the \enquote{identity function},
which in reality is the graph of the identity function.

\begin{definition}
  The identity \enquote{function} \(\funfont{id}: \alpha \to_s \alpha\) is the 
  set of pairs \(\funfont{id} = \{(x, y) \mid x = y\}\).
\end{definition}

We can also define the composition of functions as the relational composition
of the graphs of the functions.

\begin{definition}
  The composition of two functions \(f: \alpha \to_s \beta\) and \(g: \beta \to_s \gamma\)
  is the set of pairs:
  \[
...
Of course we can prove that composing functions with the identity function
is the same as the function itself.

\begin{theorem}
  The composition of a function with the identity function is the function itself.
...
function as:
  \begin{align*}
    \funfont{denote} \ \texttt{skip} &\coloneqq \funfont{id} \\
    \funfont{denote} \ (v \ \texttt{:=} \ a) &\coloneqq \{(s, t) \mid t = s[v \leftarrow a \ s]\} \\
    \funfont{denote} \ c;c' &\coloneqq \funfont{denote} \ c \circ \funfont{denote} \ c' \\
    \funfont{denote} \ (\texttt{if} \ b \ \texttt{then} \ c \ \texttt{else} \ c' \ \texttt{end}) &\coloneqq \funfont{Set.ite} \ \{(s, \square) \mid b \ s\} \ (\funfont{denote} \ c) \ (\funfont{denote} \ c') \\
    \funfont{denote} \ (\texttt{while} \ b \ \texttt{loop} \ c \ \texttt{end}) &\coloneqq \funfont{lfp} \ (W \ b \ (\funfont{denote} \ c))
  \end{align*}
  where \(W \ b \ f: (\tyfont{State} \to_s \tyfont{State}) \to (\tyfont{State} \to_s \tyfont{State})\) is:
  \[
    W \ b \ f = \funfont{Set.ite} \ \{(s, \square) \mid b \ s\} \ (f \circ g) \ \funfont{id}
  \]
\end{definition}

Of course this is only well defined if the least fix-point of \(W \ b \ f\) exists,
for this it suffices to show that \(W \ b \ f\) is monotonic, as we can then apply
the \emph{Knaster-Tarski} theorem.

\begin{theorem}
  The function \(W \ b \ f\) is monotonic.
\end{theorem}
...
\lstinputlisting{../SemanticsLean/Semantics/Maps.lean}

\subsection{Sets}

\lstinputlisting{../SemanticsLean/Semantics/Set.lean}

\subsection{IMP}

\lstinputlisting{../SemanticsLean/Semantics/Imp/Lang.lean}

\subsection{Aexp}

\lstinputlisting{../SemanticsLean/Semantics/Imp/Aexp.lean}

\subsection{Bexp}

\lstinputlisting{../SemanticsLean/Semantics/Imp/Bexp.lean}

\subsection{Big-step}

\lstinputlisting{../SemanticsLean/Semantics/Imp/BigStep.lean}

\subsection{Reflexive transitive closure}

\lstinputlisting{../SemanticsLean/Semantics/ReflTransRel.lean}

\subsection{Small-step}

\lstinputlisting{../SemanticsLean/Semantics/Imp/SmallStep.lean}

\subsection{Knaster-Tarski}

\lstinputlisting{../SemanticsLean/Semantics/KnasterTarski.lean}

\subsection{Denotational semantics}

\lstinputlisting{../SemanticsLean/Semantics/Imp/Denotational.lean}

\subsection{Equivalence}

\lstinputlisting{../SemanticsLean/Semantics/Imp/Equivalence.lean}
